...
import org.hyperic.hq.agent.bizapp.commands.CreateToken_args;
import org.hyperic.hq.agent.bizapp.commands.CreateToken_result;
import org.hyperic.hq.agent.bizapp.callback.AgentCallbackClient;
...
public class AgentClient {
    ...
    private BizappCallbackClient testProvider(String provider) throws AgentCallbackClientException {
        bizappCallback.setFetcher(new StaticProviderFetcher(new ProviderInfo(provider, "no-auth")));
        bizappCallback.bizappPing();
        return bizappCallback;
    }
    ...
    private BizappCallbackClient getConnection(String provider, boolean secure) throws AutoQuestionException, AgentCallbackClientException {
        ...
        while (true) {
            ...
            try {
                bizappCallback = this.testProvider(provider);
                log.info("testing connectivity to the unidirectional and lather servlet container: unidirectionalProvider=" + bizappCallback);
...
//                false, QPROP_NEWTRANSPORT);   
        ...
        while (true) {
            ...
            try {
                 bizappCallback = getConnection(provider, secure);
        ...
        while (true) {
            ...
            pword = this.askQuestion("What is your " + PRODUCT +
                    ...
                    QPROP_PWORD);
            try {
                if (bizappCallback.userIsValid(user, pword)) {
                    break;
                }
            } catch (AgentCallbackClientException exc) {
        ...
        if ((providerInfo = this.camCommands.getProviderInfo()) != null &&
            ...
            if (setupTokens == false) {
                ...
                try {
                    response = bizappCallback.updateAgent(providerInfo.getAgentToken(),
                            user, pword, agentIP,
                            agentPort,
                            isNewTransportAgent,
                            unidirectional);
        ...
        try {
            ...
            response = bizappCallback.registerAgent(oldAgentToken, user, pword, tokenRes.getToken(),
                                            agentIP, agentPort, ProductProperties.getVersion(), getCpuCount(),
                                            isNewTransportAgent, unidirectional);
    ...
    private void verifyAgentRunning(ServerSocket startupSock) throws AgentInvokeException {
        try {
            ...
            SYSTEM_OUT.println("- Verified the Agent is successfully running.");
    ...
    private int cmdStart(boolean force) throws AgentInvokeException {
        ...
        try {
            ...
            throw new AgentInvokeException("Unable to setup a socket to listen for Agent startup: " + exc.getMessage());
        }
        ...
        verifyAgentRunning(startupSock);
        ...
        try {
            ...
            // This should rarely (never) occur, since we just ensured things were operational.
    ...
    private static AgentClient initializeAgent(boolean generateToken) {
        ...
        if (generateToken) {
            ...
            while (initializeStartTime > (System.currentTimeMillis() - startupTimeout)) {
                try {
                    authToken = AgentClientUtil.getLocalAuthToken(tokenFile);
                    conn = new org.hyperic.hq.agent.bizapp.client.SecureAgentConnection(connIp, cfg.getListenPort(), authToken);
                    return new AgentClient(cfg, conn);
                } catch (FileNotFoundException exc) {
