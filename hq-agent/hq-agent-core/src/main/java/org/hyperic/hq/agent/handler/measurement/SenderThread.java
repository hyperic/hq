...
import org.hyperic.hq.operation.DSNList;
import org.hyperic.hq.operation.MeasurementReport;
import org.hyperic.hq.operation.MetricValue;
...
import static org.hyperic.hq.operation.MeasurementReport.MeasurementReportBuilder;
...
public class SenderThread extends AgentMonitorSimple implements Sender, Runnable {
    ...
    private final List<String> transitionQueue;
    ...
    private Set<Integer> metricDebug;
    ...
    SenderThread(Properties bootProps, AgentStorageProvider storage, MeasurementSchedule schedule, MeasurementCallback callback) throws AgentStartException {
        ...
        this.metricDebug = new HashSet<Integer>();
        ...
        if (sMetricDebug != null) {
            try {
                for (Object o : StringUtil.explode(sMetricDebug, " ")) {
                    Integer metId = new Integer((String) o);
    ...
    private void sendBatch() {
         // Before sending the data off, make sure our transition queue is empty
        ...
        // first ensure that all the data points that we send over to the server are unique
        for (; it != null && it.hasNext() && numUsed < this.maxBatchSize; numUsed++) {
            try {
                ...
                if (!didNotAlreadyExist) {
                    if (log.isDebugEnabled()) log.debug("Dropping duplicate entry for " + r);
                    numUsed--;
                }
            } catch (IOException exc) {
                this.log.error("Error accessing record -- deleting: " + exc);
            }
        }

        MeasurementReportBuilder builder = new MeasurementReportBuilder();

        for(Record record: records) {
            if (log.isDebugEnabled()) {
                this.log.debug("    Data:  d=" + record.derivedID + " r=" + record.dsnId + " t=" + record.data.getTimestamp() + " v=" + record.data);
            }
            if (this.metricDebug.contains(new Integer(record.derivedID))) {
                numDebuggedSent++;
                this.log.info("metricDebug:  Pulled DSN=" + record.dsnId + " derivedID=" + record.derivedID + " value=" + record.data.getValue() + " from tQueue -- sending");
            }

            builder.addMetric(record.derivedID, record.dsnId, record.data);
        }
        ...
        List<DSNList> clientIds = builder.buildMetricValues();

        success = false;
        try {

            List<SRN> srnList = schedule.getSrnList();
            
            if (srnList == null || srnList.size() == 0) {
            ...
            if (log.isDebugEnabled()) {
                for (SRN aSrnList : srnList) {
                    this.log.debug("    SRN: " + aSrnList.getId() + "=" + aSrnList.getRevisionNumber());
                }
            }


            if (this.agentToken == null) this.agentToken = storage.getValue(CommandsAPIInfo.PROP_AGENT_TOKEN);
 
            callback.measurementSendReport(new MeasurementReport(agentToken, clientIds, srnList));

            success = true;
            serverError = false;
        } catch (AgentCallbackException exc) {
            if (!serverError) {
                this.log.error("Error sending measurements: " + exc.getMessage());
                serverError = true;
            }
        } finally {
            if (numDebuggedSent != 0) {
                if (success) {
                    this.log.info("metricDebug:  Successfully sent " + numDebuggedSent + " debugged metrics to server");
                } else {
                    this.log.info("metricDebug:  Server reported failure when sent " + numDebuggedSent + " debugged metrics");
                }
            }
        }
    ...
    private static class Record {
        ...
        public int hashCode() {
            ...
            final Long timestamp = data.getTimestamp() * 71;
            final Integer mId = derivedID * 71;
            hashCode = 7 + (timestamp.hashCode() * 71) + (mId.hashCode() * 71);
            return hashCode;
        }
    ...
    public void processData(int dsnId, Object metricValue, int derivedID) {
        MetricValue data = (MetricValue) metricValue;
        String encodedRec;
        double val;
    ...
    private void processTransitionQueue() {
        synchronized (this.transitionQueue) {
            for (Object aTransitionQueue : this.transitionQueue) {
                String val = (String) aTransitionQueue;
