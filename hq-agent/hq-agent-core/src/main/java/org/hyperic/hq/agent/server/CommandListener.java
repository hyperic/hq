...
public class CommandListener extends AgentMonitorSimple {
    ...
    void die() throws AgentRunningException {
        if (!running) throw new AgentRunningException("CommandListener not listening");
        this.shouldDie = true;
    }
    ...
    private void gatherLastRequestTimeStatistics(long lastRequestTime) {
        ...
        if (requestTime > this.stat_maxRequestTime || this.stat_maxRequestTime == -1) {
            this.stat_maxRequestTime = requestTime;
        }

        if (requestTime < this.stat_minRequestTime || this.stat_minRequestTime == -1) {
