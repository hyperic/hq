...
implements Sender, Runnable 
{
	...
	private boolean shouldRetry(AgentCallbackClientException exc) {
		...
		if(exc.getMessage().toLowerCase().endsWith("refused")){
			...
			if(exc.getMessage().endsWith(AgentCallbackClientException.PERMISSION_DENIED_ERROR_MSG)){
				log.error("Agent is not able to communicate with the server because the agent was deleted from Hyperic server" +
						", agent will stop sending data to Hyperic server. Please resetup the agent: stop the agent and delete data directory ");
				stopSendingData.set(true);
				return false;
			}else{
				//check if there is a new certification in the server
				if((!(exc.getMessage().indexOf("Service Unavailable") != -1) &&
