...
public enum ShutdownType {
	...
	Restart(12){ 
		/**
		 * Attempts to detect the a Tanukisoft {@link WrapperManager} and request a gracefull<br>
		 * restart. If presence was not detected will resort to shutdown. 
		 */
		@Override
		public final void shutdown() {
			//attempt to detect a wrapper presence and if found, request a restart, else 
			//invoke the normal system.exit 
			if(WrapperManager.isControlledByNativeWrapper()) WrapperManager.restartAndReturn();
			else { 
	...
	private static Map<Integer,ShutdownType> m_mapReverseMapping = 
										new HashMap<Integer, ShutdownType>() ;
	 ...
	 */
	ShutdownType(int iExitCode) { this.m_iExitCode = iExitCode ; }//EOM
	 ...
	 */
	public void shutdown() { System.exit(this.m_iExitCode) ; }//EOM 
	
	/**
	 * @return {@link System#exit(int)} exit code 
	 */
	public final int exitCode() { return this.m_iExitCode ; }//EOM 
