...
public class MQSeriesCmdHandler {
	...
	public MQSeriesCmdHandler() {
		cmds.put("mqseries-mgr", new CmdInquireQMgr());
		cmds.put("mqseries-queue", new CmdInquireQ());
		cmds.put("mqseries-channel", new CmdInquireCH());
	}

	public Double getValue(Metric metric) throws PluginException, MetricNotFoundException, MetricUnreachableException {
		log.debug("getValue("+metric+")");
		...
		MQSeriesCmd cmd = (MQSeriesCmd) this.cmds.get(domain);
		log.debug("cmd='"+cmd+"' domain='"+domain+"'");

		if (mgr == null) {
			throw new MetricInvalidException("Missing " + MQSeriesCmd.PROP_MGR + " Attribute");
		}
