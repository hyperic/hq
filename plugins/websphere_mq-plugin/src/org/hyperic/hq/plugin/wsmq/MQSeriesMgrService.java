...
public class MQSeriesMgrService extends MQSeriesService {

	// XXX might need to unhardcode.
	public static final File QMGRS_PATH = new File("/var/mqm/qmgrs");

	private static final Log log = LogFactory.getLog(MQSeriesMgrService.class.getName());
	...
	public Map getProductConfig() {
		this.productConfig.setProperty(MQSeriesProductPlugin.PROP_MGR_NAME, getName());
	...
	private static String[] findQmgrs(File path) {
		...
		File[] dirs = path.listFiles(new FileFilter() {
			public boolean accept(File dir) {
				...
				if (dir.isDirectory() && new File(dir, "queues").exists()) {
					return true;
				}
				return false;
			}
	...
	private static String[] findQmgrsRegistry() throws PluginException {

		String MQ_MGR_KEY = MQSeriesProductPlugin.MQ_KEY + "\\Configuration\\QueueManager";
		RegistryKey key = null;

		try {
			key = RegistryKey.LocalMachine.openSubKey(MQ_MGR_KEY);
	...
	// XXX realmente esta clase no vale para nada...
	public static List findServices() throws PluginException {
		...
		if (GenericPlugin.isWin32()) {
			...
			if (qmgrs.length == 0) {
				String path = MQSeriesProductPlugin.getRegistryWorkPath();
		...
		for (int i = 0; i < qmgrs.length; i++) {
			...
			services.add(StringUtil.replace(qmgrs[i], "!", "."));
