...
// XXX cambiar de nombre a utils... o algo parecido...
public class MQSeriesQueueService extends MQSeriesService {
	private static Log log = LogFactory.getLog(MQSeriesQueueService.class);

	private static final int[] ATTRS = { CMQC.MQCA_Q_NAME, CMQC.MQIA_DEFINITION_TYPE };
	...
	public Map getProductConfig() {
		this.productConfig.setProperty(MQSeriesProductPlugin.PROP_Q_NAME, getName());
	...
	// XXX modificado
	public static List findQueues(String mgrName) throws PluginException {
		...
		try {
			...
			for (int i = 0; i < responses.length; i++) {
				String name;
				int type = responses[i].getIntParameterValue(CMQC.MQIA_DEFINITION_TYPE);
	...
	public static List findServices(MQSeriesMgrService mgr) throws PluginException {

		List names = findQueues(mgr.getName());

		ArrayList services = new ArrayList();

		String type = TypeBuilder.composeServiceTypeName(mgr.getServerType(), MQSeriesProductPlugin.Q_NAME);

		for (int i = 0; i < names.size(); i++) {
			MQSeriesQueueService queue = new MQSeriesQueueService(mgr);
