...
public class AppdefBossTest {
    ...
    public void setUp() throws Exception {
        agent = agentManager.createLegacyAgent("127.0.0.1", 2144, "hqadmin", "agentToken", "4.5");
       ...
       ServerType serverType = serverManager.findServerTypeByName("FileServer");
        ...
        for(int i=1;i<= 2078;i++) {
            service = serviceManager.createService(authzSubjectManager.getOverlordPojo(), server.getId(),
    ...
    @Test
    public void testDeletePlatform() throws SessionNotFoundException, SessionTimeoutException,
        ApplicationException, VetoException, InterruptedException {
        //measurementManager.disableMeasurementsForDeletion(authzSubjectManager.getOverlordPojo(), agent, new AppdefEntityID[] {platform.getEntityId(),service.getEntityId()});
        platform = platformManager.findPlatformByFqdn(authzSubjectManager.getOverlordPojo(), "Platform1");
        int sessionId = sessionManager.put(authzSubjectManager.getOverlordPojo());
