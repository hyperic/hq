    ...
    extends TestCase
{
    ...
    /**
     * Verifies that nothing blows up if an Exception occurs retrieving stored measurement values when the trigger is initialized
    ...
    /**
     * Verifies that nothing blows up if there are no stored measurement values when the trigger is initialized
     * @throws EncodingException
     * @throws InvalidTriggerDataException
     */
    public void testInitializeNoLastValue() throws EncodingException, InvalidTriggerDataException {
        ...
        EasyMock.expect(dataManagerLocal.getLastHistoricalData(measurement)).andReturn(null);
        replay();
        initTrigger();
        verify();
        assertNull(trigger.getLast());
    ...
    /**
     * Verifies that first event is processed correctly if trigger has no initial value
     * @throws EncodingException
     * @throws InvalidTriggerDataException
    ...
    public void testProcessEventWrongMeasurementId() throws EncodingException, InvalidTriggerDataException {
        ...
        EasyMock.expect(measurementManagerLocal.getMeasurement(MEASUREMENT_ID)).andReturn(new Measurement());
        EasyMock.expect(dataManagerLocal.getLastHistoricalData(measurement)).andReturn(new MetricValue(2));
        replay();
        initTrigger();
        try {
            trigger.processEvent(event);
    ...
    public void testProcessOlderValueChange() throws EventTypeException, EncodingException, InvalidTriggerDataException
    {
        ...
        EasyMock.expect(dataManagerLocal.getLastHistoricalData(measurement)).andReturn(initialValue);
     ...
     * @throws EncodingException
     */
    public void testProcessValueChangedFromInitial() throws EventTypeException,
                                                    EncodingException,
