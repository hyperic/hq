...
then
   ...
   cd $AGENT_BASE_DIR
   wget http://10.0.0.240/raid/release/candidates/hq/$AGENT_BINARY_NAME
   if [ -d $AGENT_HOME ]
   then
     rm -rf $AGENT_HOME
   fi
   tar -zxf $AGENT_BINARY_NAME
   ...
   wget http://10.0.0.240/raid/release/candidates/qatools/testserver-plugin.xml
