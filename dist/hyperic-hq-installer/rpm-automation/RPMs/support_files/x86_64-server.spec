...
%define HQ_Component_Version    4.5.0
%define HQ_Component_Build	M6-x86-64-linux
%define HQ_Component_Release   	1 
%define HQ_Component_Build_Type M6
...
if [ -d /opt/hyperic/server-current ] && cd /opt/hyperic/server-current; then
        ...
        #echo "%{prefix}/server-current/hqdb/bin/postgres -D %{prefix}/server-current/hqdb/data -i" > %{prefix}/server-%{version}/hqdb/data/postmaster.opts
        cd $RPM_BUILD_ROOT/%{prefix}
        /bin/su hyperic -c "/bin/ln -snf server-%{version}.%{HQ_Component_Build_Type} server-current"
else
	cd %{prefix}/%{name}
...
%{__cp} %{prefix}/%{name}/installer/logs/hq-install.log %{prefix}/server-%{version}.%{HQ_Component_Build_Type}/hq-install.log
%{__rm} -Rf %{prefix}/%{name}

%postun

if [ ! -f /etc/init.d/hyperic-hq-server ]; then
	%{__rm} -Rf %{prefix}/server-current %{prefix}/server-%{version}.%{HQ_Component_Build_Type}
fi
...
if [ -f /etc/init.d/hyperic-hq-server ] && [ -f /tmp/hyperic-hq-server-was-running-%{version}-%{release} ]; then
    ...
    echo "The installation log can be found in %{prefix}/server-%{version}.%{HQ_Component_Build_Type}/hq-install.log."
...
* Fri Nov  7 2008 Hyperic Support <support@hyperic.com> 3.2.6
...
* Thu Apr 17 2008 Hyperic Support <support@hyperic.com> 3.2.3-x86_64-linux-1
* Tue Mar 25 2008 Hyperic Support <support@hyperic.com> 3.2.2-x86_64-linux-1
* Thu Feb 28 2008 Hyperic Support <support@hyperic.com> 3.2.1-x86_64-linux-1
* Wed Jan 29 2008 Hyperic Support <support@hyperic.com> 3.2.0-x86_64-linux-1
- Initial release.
