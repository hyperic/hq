 ...
 *
 */
public class HQUGroovyPagesTemplateEngine  extends ResourceAwareTemplateEngine implements ApplicationContextAware, ServletContextAware, InitializingBean {
    private static final String GENERATED_GSP_NAME_PREFIX = "gsp_script_";
	private static final Log LOG = LogFactory.getLog(HQUGroovyPagesTemplateEngine.class);
    private Map<String, HQUGroovyPageMetaInfo> pageCache = new ConcurrentHashMap<String, HQUGroovyPageMetaInfo>();
    ...
    private Map<String, HQUGroovyPageMetaInfo> precompiledCache = new ConcurrentHashMap<String, HQUGroovyPageMetaInfo>();

    public HQUGroovyPagesTemplateEngine() {
    }

    public HQUGroovyPagesTemplateEngine(ServletContext servletContext) {
    ...
    public int[] calculateLineNumbersForPage(ServletContext context,String url) {
        try {
        	...
        	if(t != null && t instanceof HQUGroovyPageTemplate) {
        		return ((HQUGroovyPageTemplate)t).getMetaInfo().getLineNumbers();
        	}
        } catch (Exception e) {
    ...
    public Template createTemplate(Resource resource) {
        ...
        if(pageCache.containsKey(name)) {
            HQUGroovyPageMetaInfo meta = pageCache.get(name);
            ...
            if(isGroovyPageReloadable(resource, meta)) {
                ...
                return new HQUGroovyPageTemplate(meta);
            }
        }
        else {
            try {
    ...
    private HQUGroovyPageTemplate createTemplateFromPrecompiled(String uri) {
    	if(isReloadEnabled()) {
    		return null;
    	}
    	HQUGroovyPageTemplate t=createTemplateFromPrecompiled(uri, uri);
    	if(t==null) {
	...
	private HQUGroovyPageTemplate createTemplateFromPrecompiled(String originalUri, String uri) {
		if(precompiledGspMap != null) {
    		HQUGroovyPageMetaInfo meta = precompiledCache.get(uri);
    		if(meta != null) {			
    			return new HQUGroovyPageTemplate(meta);
    		}
    		...
    		if(gspClassName != null) {
				...
				if(gspClass != null) {
					meta = new HQUGroovyPageMetaInfo(gspClass);
					meta.setJspTagLibraryResolver(jspTagLibraryResolver);
					meta.setTagLibraryLookup(tagLibraryLookup);
	    			if(LOG.isDebugEnabled()) {
	    				LOG.debug("Adding GSP class HQUGroovyPageMetaInfo in cache for uri " + uri + " classname is " + gspClassName);
	    			}    			
    ...
    public Template createTemplate(InputStream inputStream) {
        ...
        HQUGroovyPageMetaInfo metaInfo = buildPageMetaInfo(inputStream, null, null);

        return new HQUGroovyPageTemplate(metaInfo);
    }
    ...
    protected Template createTemplate(InputStream inputStream, Resource resource, String pageName) {
        HQUGroovyPageMetaInfo metaInfo = buildPageMetaInfo(inputStream, resource, pageName);
        return new HQUGroovyPageTemplate(metaInfo);
    }
     ...
     * @param meta The current HQUGroovyPageMetaInfo instance
     * @return True if it is reloadable
     */
    private boolean isGroovyPageReloadable(Resource resource, HQUGroovyPageMetaInfo meta) {
    ...
    /**
     * Constructs a HQUGroovyPageMetaInfo instance which holds the script class, modified date and so on
     *
     * @param inputStream The InputStream to construct the HQUGroovyPageMetaInfo instance from
     ...
     * @return The HQUGroovyPageMetaInfo instance
     */
    protected HQUGroovyPageMetaInfo buildPageMetaInfo(InputStream inputStream, Resource res, String pageName) {
        ...
        // Make a new metaInfo
        HQUGroovyPageMetaInfo metaInfo = createPageMetaInfo(parser, lastModified, in);
        try {
     ...
     */
    private Class compileGroovyPage(InputStream in, String name, String pageName, HQUGroovyPageMetaInfo metaInfo) {
    ...
    /**
     * Creates a HQUGroovyPageMetaInfo instance from the given Parse object, and initialises it with the the specified
     * last modifed date and InputStream
     *
     * @param parse The Parse object
     * @param lastModified The last modified date
     * @param in The InputStream instance
     * @return A HQUGroovyPageMetaInfo instance
     */
    private HQUGroovyPageMetaInfo createPageMetaInfo(GroovyPageParser parse, long lastModified, InputStream in) {
        HQUGroovyPageMetaInfo pageMeta = new HQUGroovyPageMetaInfo();
