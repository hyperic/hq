...
public class RegisterAgentServiceImpl implements RegisterAgentService {
    ...
    public void registerAgentRequest(RegisterAgentRequest registerAgentRequest) throws AgentConnectionException {
        try {
            this.serverOperationServiceValidator.checkUserCanManageAgent(registerAgentRequest.getUsername(), registerAgentRequest.getPassword(), "register", registerAgentRequest.getAgentIp());
        } catch (PermissionException e) {
            throw new AgentConnectionException("Error creating agent", e);
        }

        String agentIP = registerAgentRequest.getAgentIp();
        int port = registerAgentRequest.getAgentPort();
        String version = registerAgentRequest.getVersion();
        boolean isNewTransportAgent = registerAgentRequest.isNewTransportAgent();
        boolean unidirectional = registerAgentRequest.isUnidirectional();
        ...
        try {
            this.serverOperationServiceValidator.testAgentConn(agentIP, port, registerAgentRequest.getAuthToken(), isNewTransportAgent, unidirectional);
        } catch (AgentConnectionException e) {
            throw new AgentConnectionException("Error creating agent", e);
        }

        boolean isOldAgentToken = true;
        String agentToken = registerAgentRequest.getAgentToken();
        ...
        try {
            Agent origAgent = isOldAgentToken? this.agentManager.getAgent(agentToken) : this.agentManager.getAgent(agentIP, port);
            ...
            logger.info("Found pre-existing agent during agent registration. " + "Updating agent information for "
                    + agentIP + ":" + port + "; new transport=" + isNewTransportAgent + "; unidirectional=" + unidirectional);

            if (isOldAgentToken) {
                if (isNewTransportAgent) {
                    this.agentManager.updateNewTransportAgent(agentToken, agentIP, port, registerAgentRequest.getAuthToken(), version, unidirectional);
                } else {
                    this.agentManager.updateLegacyAgent(agentToken, agentIP, port, registerAgentRequest.getAuthToken(), version);
                }
            } else {
                if (isNewTransportAgent) {
                    this.agentManager.updateNewTransportAgent(agentIP, port, registerAgentRequest.getAuthToken(), agentToken, version, unidirectional);
                } else {
                    this.agentManager.updateLegacyAgent(agentIP, port, registerAgentRequest.getAuthToken(), agentToken, version);
                }
            }
        } catch (AgentNotFoundException exc) {
            logger.info("Registering agent at " + agentIP + ":" + port + "" + "; new transport=" + isNewTransportAgent
                    + "; unidirectional=" + unidirectional);
            try {
                if (isNewTransportAgent) {
                    this.agentManager.createNewTransportAgent(agentIP, port, registerAgentRequest.getAuthToken(), agentToken, version, unidirectional);
                } else {
                    this.agentManager.createLegacyAgent(agentIP, port, registerAgentRequest.getAuthToken(), agentToken, version);
                }
