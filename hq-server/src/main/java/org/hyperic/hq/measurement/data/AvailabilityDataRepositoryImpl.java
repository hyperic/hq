...
public class AvailabilityDataRepositoryImpl implements AvailabilityDataCustom {
    ...
    public List<AvailabilityDataRLE> findByMeasurements(List<Integer> mids) {
    ...
    public List<AvailabilityDataRLE> getHistoricalAvails(Integer[] mids, long start, long end,
                                                         boolean descending) {
        ...
        for (int i = 0; i < list.size(); i += BATCH_SIZE) {
            ...
            rtn.addAll(entityManager.createQuery(sql, AvailabilityDataRLE.class)
                ...
                .setParameter("mids", list.subList(i, last)).getResultList());
        }
        return rtn;
    }

    @SuppressWarnings("unchecked")
    public Map<Integer, TreeSet<AvailabilityDataRLE>> getHistoricalAvailMap(Integer[] mids,
                                                                            final long after,
    ...
    public List<Object[]> findAggregateAvailability(Integer[] mids, long start, long end) {
        if (mids.length == 0) {
            // Nothing to do
