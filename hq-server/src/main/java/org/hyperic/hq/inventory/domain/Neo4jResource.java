...
public class Neo4jResource implements Resource {
    ...
    @Transient
    @ManyToOne
    @RelatedTo(type = RelationshipTypes.IS_A, direction = Direction.OUTGOING, elementClass = Neo4jResourceType.class)
    ...
    public Set<ResourceRelationship> getRelationships(Resource entity, String name, Direction direction) {
       return convertRelationships((Neo4jResource)entity, getUnderlyingState().getRelationships(
                    DynamicRelationshipType.withName(name)));
    }
    
    @SuppressWarnings("unchecked")
    private Set<ResourceRelationship> convertRelationships(Neo4jResource entity, 
    ...
    public boolean isRelatedTo(Resource resource, String relationName) {
        ...
        for (Node related : relationTraverser) {
            if (related.equals(((Neo4jResource)resource).getUnderlyingState())) {
                return true;
            }
        }
        return false;
    ...
    public ResourceRelationship relateTo(Resource resource, String relationName) {
        ...
        return (ResourceRelationship) this.relateTo((Neo4jResource)resource, Neo4jResourceRelationship.class, relationName);
    }
    ...
    public void removeRelationships(Resource entity, String name, Direction direction) {
        for (ResourceRelationship relation : getRelationships(entity, name, direction)) {
            ((Neo4jResourceRelationship)relation).getUnderlyingState().delete();
        }
    }
    ...
    public ResourceRelationship getRelationshipTo(Resource resource, String relationName) {
        Set<ResourceRelationship> relations = convertRelationships((Neo4jResource)resource, getUnderlyingState().getRelationships(
    ...
    private void setConfig(Neo4jConfig config, ConfigType type) {
    ...
    public void setControlConfig(Config config) {
        setConfig((Neo4jConfig)config, ConfigType.CONTROL);
    }
    ...
    public void setMeasurementConfig(Config config) {
        setConfig((Neo4jConfig)config, ConfigType.MEASUREMENT);
    }
    ...
    public void setProductConfig(Config config) {
        setConfig((Neo4jConfig)config, ConfigType.PRODUCT);
    }
