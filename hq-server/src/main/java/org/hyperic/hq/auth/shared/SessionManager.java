...
//    	return sessionId;
     ...
     * @return The user id
     */
    public Integer getId(int sessionId) 
        throws SessionNotFoundException, SessionTimeoutException
    {
...
//        return getSubject(sessionId).getId();
    ...
    {
        Object currentPrincipal = getCurrentPrincipal();
        if (currentPrincipal instanceof HQUserDetails) {
            HQUserDetails userDetails = (HQUserDetails)currentPrincipal;
            if (null != userDetails) {
                return userDetails.getAuthzSubject();
            }
        }
        return null;
//        AuthSession session = (AuthSession)_cache.get(sessionId);
