...
public class Tomcat55RuntimeADPlugin 
    extends ServletDiscoveryPlugin
{                                                  
    private static final String CONNECTOR_OBJ = "Catalina:type=GlobalRequestProcessor,";

    private static final String WEBAPP_SERVICE_TYPENAME =
        ServletProductPlugin.TOMCAT_SERVER_NAME + " " +
        ServletProductPlugin.TOMCAT_VERSION_55 + " " +
        ServletProductPlugin.WEBAPP_NAME;

    private static final String CONNECTOR_SERVICE_TYPENAME =
        ServletProductPlugin.TOMCAT_SERVER_NAME + " " +
        ServletProductPlugin.TOMCAT_VERSION_55 + " " +
        ServletProductPlugin.CONNECTOR_NAME;

    private static Log log = LogFactory.getLog("Tomcat55RuntimeADPlugin");

    public Tomcat55RuntimeADPlugin () {}
    ...
    private ArrayList discoverConnectors(Manifest mBeanInfo,
                                         String serviceTypeName)
    {
        ...
        for (Iterator i = objectNames.iterator(); i.hasNext();) {
            ...
            if (objectName.startsWith(CONNECTOR_OBJ)) {
                //Found one
                String name = 
                    objectName.substring(objectName.indexOf("name=") + 5);
                ...
                try {
                    ...
                    metricConfig.setValue("connectorName", name);
