...
%define HQ_Component_Build	1053-x86_64-linux
...
Source0:        %{HQ_Component_Name}-%{HQ_Component_Version}-%{HQ_Component_Build}.tgz
Vendor:		Hyperic, Inc.
License:        GPLv2
...
%setup -T -b 0 -n %{HQ_Component_Name}
...
%{__rm} -f installer-%{HQ_Component_Version}/lib/sigar-x86-winnt.lib
...
if [ -d /opt/hyperic/server-current ] && cd /opt/hyperic/server-current; then
	...
	cd %{prefix}/%{name}
        installer-%{version}/data/hqdb/tune-os.sh > /dev/null 2> /dev/null
...
if [ ! -f /etc/init.d/hyperic-hq-server ]; then
	%{__rm} -Rf %{prefix}/server-current %{prefix}/server-%{version}
fi
