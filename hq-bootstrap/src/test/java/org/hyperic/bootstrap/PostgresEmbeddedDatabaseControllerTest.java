...
public class PostgresEmbeddedDatabaseControllerTest {
    ...
    public void setUp() {
        ...
        this.embeddedDBController = new PostgresEmbeddedDatabaseController(serverHome, processManager, osInfo);
    }
    
    @Test
    public void testStartBuiltInDB() throws Exception {
        EasyMock.expect(processManager.getPidFromPidFile(serverHome + "/hqdb/data/postmaster.pid")).andReturn(-1l);
        EasyMock.expect(osInfo.getName()).andReturn("Mac OS X");
        EasyMock.expect(processManager.executeProcess(EasyMock.aryEq(new String[] { serverHome + "/bin/db-start.sh" }),
            EasyMock.eq(serverHome), EasyMock.eq(false))).andReturn(0);
    ...
    public void testStartAlreadyRunning() throws SigarException, IOException {
        EasyMock.expect(processManager.getPidFromPidFile(serverHome + "/hqdb/data/postmaster.pid")).andReturn(-5678l);
        replay();
        boolean dbStarted = embeddedDBController.startBuiltInDB();
        verify();
        assertTrue(dbStarted); 
    ...
    public void testStopBuiltInDB() throws Exception {
        EasyMock.expect(processManager.getPidFromPidFile(serverHome + "/hqdb/data/postmaster.pid")).andReturn(123l);
        EasyMock.expect(osInfo.getName()).andReturn("Mac OS X");
        EasyMock.expect(processManager.executeProcess(EasyMock.aryEq(new String[] { serverHome + "/bin/db-stop.sh" }),
            EasyMock.eq(serverHome), EasyMock.eq(false))).andReturn(0);
