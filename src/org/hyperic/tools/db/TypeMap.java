...
{
    private String    m_strGenericType;
    private Hashtable m_mapDBType = new Hashtable();
    
    protected TypeMap(Node node) throws SAXException
    {
        if(TypeMap.isTypeMap(node) == true)
        {
            ...
            for(int iType = 0;iType < type.getLength();iType ++)
            {
                ...
                {
                    this.m_strGenericType = strValue;
                    ...
                    for(int iMap = 0;iMap < listMaps.getLength();iMap ++)
                    {
                        ...
                        if(node.getNodeName().equalsIgnoreCase("map") == true)
                        {
                            ...
                            this.m_mapDBType.put(strDB, strMappedType);
                        }
                    }
                }
            }
        }
        else throw new SAXException("node is not a TypeMap.");
    }
    
    protected boolean contains(String database)
    {
        return this.m_mapDBType.contains(database);
    }
    
    protected String getGenericType()
    {
        return this.m_strGenericType;
    }
    ...
    public String getMappedType(String type, String database)
    {
        ...
        if(this.m_strGenericType.equalsIgnoreCase(type) == true)
            strResult = (String)this.m_mapDBType.get(database);
        else
            strResult = null;
    ...
    {
        return this.getMappedType(type, JDBC.toName(database));
    }
