...
public class LiveDataCommandsClientFactory {
    ...
    public LiveDataCommandsClient getClient(AppdefEntityID aid) 
        ...
        AgentValue agentValue = AgentManagerEJBImpl.getOne().getAgent(aid);

        return getClient(agentValue);
    }

    public LiveDataCommandsClient getClient(String agentToken) 
        throws AgentNotFoundException {
        
        AgentValue agentValue = AgentManagerEJBImpl.getOne().getAgent(agentToken);
        return getClient(agentValue);
    }
    
    private LiveDataCommandsClient getClient(AgentValue agentValue) {
        if (agentValue.isNewTransportAgent()) {
            AgentProxyFactory factory = HQApp.getInstance().getAgentProxyFactory();
            
            // get handle on agent, since API doesn't deal with value objects
            Agent agent = AgentManagerEJBImpl.getOne().findAgentPojo(agentValue.getId());
            return new LiveDataCommandsClientImpl(agent, factory);
        } else {
            return new LegacyLiveDataCommandsClientImpl(
                    new SecureAgentConnection(agentValue));            
        }         
    }
