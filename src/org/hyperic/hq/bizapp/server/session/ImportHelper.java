...
//        }
        ...
        throws BatchImportException
    {
        ...
        ServerValue aServer = new ServerValue();
        aServer.setName(name);
        aServer.setServerType(sType);
        aServer.setInstallPath(server.getInstallPath());
        
        // Set the AI identifier if it is specified in the import file.  The 
        // import script should only have this set if the import file was
        // generated by the 'resource export' command in the shell.

        aServer.setAutoinventoryIdentifier(server.getAutoinventoryIdentifier());
        ...
        setCProps(aServer.getEntityId(), server.getCustomProps(), name,
                  typeName);
        return aServer;
    }
