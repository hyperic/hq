...
public class ControlBossImpl implements ControlBoss {
	// Our log instance
	private final static Log log = LogFactory.getLog(ControlBossImpl.class
			.getName());
	...
	public void doGroupAction(int sessionId, AppdefEntityID groupEnt,
		...
		controlManager.doGroupAction(subject, groupEnt, action, orderSpec,
				schedule);
	}
	...
	public void doGroupAction(int sessionId, AppdefEntityID groupEnt,
		...
		controlManager
				.doGroupAction(subject, groupEnt, action, args, orderSpec);
	}
	...
	public boolean isControlSupported(int sessionId, AppdefResourceValue res)
		...
		return controlManager.isControlSupported(subject, res.getEntityId(),
	...
	public boolean isControlSupported(int sessionId, AppdefEntityTypeID tid)
		...
		return controlManager.isControlSupported(subject, tid
				.getAppdefResourceType().getName());
	}
	...
	public boolean isControlEnabled(int sessionId)
		...
		Map<String, AppdefEntityID> platTypes = appdefManager
		...
		Map<String, AppdefEntityTypeID> svrTypes = appdefManager
				.getControllableServerTypes(subject);

		if (svrTypes.size() > 0) {
			return true;
		}

		Map<String, AppdefEntityTypeID> svcTypes = appdefManager
				.getControllableServiceTypes(subject);
	 ...
	 */
	public PageList<ControlSchedule> findScheduledJobs(int sessionId,
			AppdefEntityID id, PageControl pc) throws PluginException,
	 ...
	 */
	public PageList<ControlHistory> findJobHistory(int sessionId,
			AppdefEntityID id, PageControl pc) throws PluginException,
			ApplicationException, PermissionException,
			SessionNotFoundException, SessionTimeoutException {
		AuthzSubject subject = sessionManager.getSubject(sessionId);
	 ...
	 */
	public PageList<ControlHistory> findGroupJobHistory(int sessionId,
			AppdefEntityID id, int batchJobId, PageControl pc)
		...
		return controlScheduleManager.findGroupJobHistory(subject, batchJobId,
				id, pc);
	}
	 ...
	 */
	public PageList<ControlHistory> getRecentControlActions(int sessionId,
			int rows, long window) throws ApplicationException,
			PermissionException, SessionNotFoundException,
			SessionTimeoutException {
		AuthzSubject subject = sessionManager.getSubject(sessionId);

		return controlScheduleManager.getRecentControlActions(subject, rows,
				window);
	}
	 ...
	 */
	public PageList<ControlSchedule> getPendingControlActions(int sessionId,
			int rows) throws ApplicationException, PermissionException,
			SessionNotFoundException, SessionTimeoutException {
		AuthzSubject subject = sessionManager.getSubject(sessionId);
	 ...
	 */
	public PageList<ControlFrequencyValue> getOnDemandControlFrequency(
			int sessionId, int numToReturn) throws ApplicationException,
			PermissionException, ApplicationException,
			SessionNotFoundException, SessionTimeoutException {
		AuthzSubject subject = sessionManager.getSubject(sessionId);

		return controlScheduleManager.getOnDemandControlFrequency(subject,
				numToReturn);
	}
	 ...
	 */
	public Map<String, AppdefEntityID> findControllablePlatformTypes(
	 ...
	 * @throws PermissionException
	 */
	public Map<String, AppdefEntityTypeID> findControllableServerTypes(
			int sessionID) throws SessionNotFoundException,
			SessionTimeoutException, PermissionException {
		AuthzSubject subject = sessionManager.getSubject(sessionID);
	 ...
	 * @throws PermissionException
	 */
	public Map<String, AppdefEntityTypeID> findControllableServiceTypes(
			int sessionID) throws SessionNotFoundException,
			SessionTimeoutException, PermissionException {
		AuthzSubject subject = sessionManager.getSubject(sessionID);
	 ...
	 * @throws PermissionException
	 */
	public Map<String, AppdefEntityID> findControllableResourceNames(
			int sessionID, AppdefEntityTypeID aetid)
		...
		case AppdefEntityConstants.APPDEF_TYPE_PLATFORM:
			result = appdefManager.getControllablePlatformNames(subject, aetid
					.getID());
			groupType = AppdefEntityConstants.APPDEF_TYPE_GROUP_COMPAT_PS;
			break;
		case AppdefEntityConstants.APPDEF_TYPE_SERVER:
			result = appdefManager.getControllableServerNames(subject, aetid
					.getID());
			groupType = AppdefEntityConstants.APPDEF_TYPE_GROUP_COMPAT_PS;
			break;
		case AppdefEntityConstants.APPDEF_TYPE_SERVICE:
			result = appdefManager.getControllableServiceNames(subject, aetid
					.getID());
		...
		try {
			...
			List<AppdefResourceValue> groups = aboss.findCompatInventory(
					sessionID, groupType,
					AppdefEntityConstants.APPDEF_TYPE_GROUP, aetid.getType(),
					aetid.getID(), null, null, PageControl.PAGE_ALL);

			for (Iterator<AppdefResourceValue> i = groups.iterator(); i
					.hasNext();) {
