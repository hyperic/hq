...
public class ViewAction extends TilesAction {
   ...
   throws Exception {
        ...
        List chartList = null;
		try {
			chartList = StringUtil.explode(dashPrefs
		...
		if (chartList != null) {
			...
			for (Iterator i = chartList.iterator(); i.hasNext();) {
				...
				List chart = (List) StringUtil.explode((String) i.next(), ",");
						             					  ...
						             					  ContextUtils.getAppdefBoss(ctx))) 
				{
					...
					// the name might be generated by user input, we need to make
					// sure
					// their delimiters' presence in the names are deserialized from
					// the
					// preference system
			...
			context.putAttribute("charts", new ArrayList());
		}
