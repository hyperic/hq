 ...
 * 
 */
public class EditConfigPropertiesAction extends BaseAction {

    public static final String ERR_NOMSG
        ...
        try {
            ...
            for (i = 0; i < numConfigs; i++) {
                try {
                    ...
                    else if (cfgTypes[i].equals(
                                 ProductPlugin.TYPE_RESPONSE_TIME)) {
            ...
            for ( i=0; i<numConfigs; i++ ) {
                ...
                if ( i == ProductPlugin.CFGTYPE_IDX_PRODUCT ) {
                    ...
                    if ( i == ProductPlugin.CFGTYPE_IDX_CONTROL 
                         && !aeid.isService() && !aeid.isServer() && !aeid.isPlatform() ) {
                    ...
                    prefix = cfgTypes[i] + ".";
                    Boolean wereChanges 
                        = BizappUtils.populateConfig(request,
                                                     prefix,
                                                     schemas[i], 
                                                     newConfigs[i], 
                                                     oldConfigs[i]);
                    if ( wereChanges == null ) {
                        ...
                        // this is a service and the config prop names for the service
                        // are the same as those for the server, and the service has 
                        // never been configured, then we won't think anything has
                    ...
                    case ProductPlugin.CFGTYPE_IDX_MEASUREMENT:
                            ...
                            (allConfigs.getMetricConfig() != null))
                        {
                            ...
                            if (aeid.isServer()) {
                                ...
                                rollback = false;
                                runtimeAI = true; //XXX !((ServiceValue)updatedResource).getWasAutodiscovered();
                            }
