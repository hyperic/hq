...
<hq:constant classname="org.hyperic.hq.appdef.shared.AppdefEntityConstants" symbol="APPDEF_TYPE_PLATFORM" var="PLATFORM" />
<hq:constant classname="org.hyperic.hq.appdef.shared.AppdefEntityConstants" symbol="APPDEF_TYPE_SERVER" var="SERVER" />
<hq:constant classname="org.hyperic.hq.appdef.shared.AppdefEntityConstants" symbol="APPDEF_TYPE_SERVICE" var="SERVICE" />
<hq:constant classname="org.hyperic.hq.appdef.shared.AppdefEntityConstants" symbol="APPDEF_TYPE_APPLICATION" var="APPLICATION" />
<hq:constant classname="org.hyperic.hq.appdef.shared.AppdefEntityConstants" symbol="APPDEF_TYPE_GROUP" var="GROUP" />
...
<html:form onsubmit="selectAllOptions()" action="/resource/${Resource.entityId.typeName}/inventory/EditConfigProperties">
	...
	<c:if test="${monitorHelp != null}">
		<table width="100%">
			<tr>
				<td class="ConfigPropHeaderHelp" colspan="4">
					<fmt:message key="resource.common.inventory.configProps.EnableMonitoring" />
				</td>
			</tr>
		</table>
	</c:if>
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0" class="BlockBg">
		<tr>
			<td class="ConfigPropHeader" colspan="4">
				<fmt:message key="resource.common.inventory.configProps.Shared" />
			</td>
		</tr>
		<tr>
			<c:if test="${productConfigOptionsCount == 0}">
				<td width="100%" colspan="4">
					<fmt:message key="resource.common.inventory.configProps.Shared.zeroLength" />
				</td>
				<td></td>
			</c:if>
			<logic:iterate id="resourceConfigOption" indexId="ctr" name="org.apache.struts.taglib.html.BEAN" property="resourceConfigOptions">
				<td width="25%" class="BlockLabel">
					<c:if test="${resourceConfigOption.optional == false}">
						<html:img page="/images/icon_required.gif" width="9" height="9" border="0" />
					</c:if>
					<bean:write name="resourceConfigOption" property="shortOption" />
					</br>
					<span class="CaptionText">
						<c:out value="${resourceConfigOption.description}" />
					</span>
				</td>
				<c:choose>
					<c:when test="${resourceConfigOption.isEnumeration == false && resourceConfigOption.isBoolean == false}">
						<td width="25%" class="BlockContent">
							...
							<input type="<c:out value="${inputType}" />" size="35" name="<c:out value="${resourceConfigOption.option}" />" value="<c:out value="${resourceConfigOption.value}" />" />
						</td>
					</c:when>
		...
		<tr>
			<td class="ConfigPropHeader" colspan="4">
				<fmt:message key="resource.common.inventory.configProps.Monitoring" />
			</td>
		</tr>
		<tr>
			<c:if test="${monitorConfigOptionsCount == 0 && serverBasedAutoInventory != 1}">
				<td width="100%" colspan="4">
					<i><fmt:message key="resource.common.inventory.configProps.Monitoring.zeroLength" /></i>
				</td>
				<td></td>
			</c:if>
			<logic:iterate id="monitorConfigOption" indexId="ctr" name="org.apache.struts.taglib.html.BEAN" property="monitorConfigOptions">
				<td width="25%" class="BlockLabel">
					...
					<span class="CaptionText">
						<c:out value="${monitorConfigOption.description}" />
					</span>
				</td>
				<c:choose>
					<c:when test="${monitorConfigOption.isEnumeration == false && monitorConfigOption.isBoolean == false}">
						<td width="25%" class="BlockContent">
							<c:choose>
								<c:when test="${monitorConfigOption.isSecret == true }">
									<c:set var="inputType" value="password" />
								</c:when>
								<c:otherwise>
									<c:set var="inputType" value="text" />
							...
							<input type="<c:out value="${inputType}" />" size="35" name="<c:out value='${monitorConfigOption.option}' />" value="<c:out value='${monitorConfigOption.value}' />" />
						</td>
					</c:when>
			...
			<c:if test="${entityId.type == 3}">
				...
				<tr>
					...
					<logic:iterate id="rtConfigOption" indexId="ctr" name="org.apache.struts.taglib.html.BEAN" property="rtConfigOptions">
						<td width="25%" class="BlockLabel">
							<c:if test="${rtConfigOption.optional == false}">
								<html:img page="/images/icon_required.gif" width="9" height="9" border="0" />
							</c:if>
							<bean:write name="rtConfigOption" property="shortOption" />
							</br>
							<span class="CaptionText">
								<c:out value="${rtConfigOption.description}" />
							</span>
						</td>
						<c:choose>
							<c:when test="${rtConfigOption.isArray == false && rtConfigOption.isEnumeration == false && rtConfigOption.isBoolean == false}">
								<td width="25%" class="BlockContent">
									<c:choose>
										<c:when test="${rtConfigOption.isSecret == true }">
											<c:set var="inputType" value="password" />
										</c:when>
										<c:otherwise>
											<c:set var="inputType" value="text" />
									...
									<input type="<c:out value="${inputType}" />" size="35" name="<c:out value='${rtConfigOption.option}' />" value="<c:out value='${rtConfigOption.value}' />" />
								</td>
							</c:when>
									...
									<table width="100%" cellspacing="0" cellpadding="0" border="0">
										<tr>
											...
											<td width="100%" id="<c:out value="${rtConfigOption.option}"/>Nav">
												<div id="<c:out value="${rtConfigOption.option}"/>Up">
													<html:img page="/images/dash_movecontent_up-off.gif" width="20" height="20" alt="" border="0" />
												</div>
												<html:img page="/images/spacer.gif" width="1" height="10" border="0" />
												<div id="<c:out value="${rtConfigOption.option}"/>Down">
													<html:img page="/images/dash_movecontent_dn-off.gif" width="20" height="20" alt="" border="0" />
												</div>
												<html:img page="/images/spacer.gif" width="1" height="20" border="0" />
												<div id="<c:out value="${rtConfigOption.option}"/>Delete">
													<html:img page="/images/dash_movecontent_del-off.gif" width="20" height="20" alt="" border="0" />
												</div>
											</td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td>
												<html:link href="#" onclick="addItem('${rtConfigOption.option}')">
													<html:img page="/images/dash_movecontent_add-on.gif" width="20" border="0" />
												</html:link>
											</td>
										</tr>
									</table>
		...
		<c:if test="${controlConfigOptionsCount != null && controlConfigOptionsCount != 0}">
			<c:if test="${entityId.type != 1}">
				<tr valign="top">
					<td class="ConfigPropHeader" colspan="4">
						<fmt:message key="resource.common.inventory.configProps.Control" />
					</td>
				</tr>
				<tr valign="top">
					<c:if test="${controlConfigOptionsCount == 0}">
						<td width="100%" colspan="4">
							<i><fmt:message key="resource.common.inventory.configProps.Control.zeroLength" /></i>
						</td>
						<td></td>
					</c:if>
					<logic:iterate id="controlConfigOption" indexId="ctr" name="org.apache.struts.taglib.html.BEAN" property="controlConfigOptions">
						<td width="25%" class="BlockLabel">
							<c:if test="${controlConfigOption.optional == false}">
								<html:img page="/images/icon_required.gif" width="9" height="9" border="0" />
							</c:if>
							<bean:write name="controlConfigOption" property="shortOption" />
							</br>
							<span class="CaptionText">
								<c:out value="${controlConfigOption.description}" />
							</span>
						</td>
						<c:choose>
							<c:when test="${controlConfigOption.isEnumeration == false && controlConfigOption.isBoolean == false }">
								<td width="25%" class="BlockContent">
									<c:choose>
										<c:when test="${controlConfigOption.isSecret == true }">
											<c:set var="inputType" value="password" />
										</c:when>
										<c:otherwise>
											<c:set var="inputType" value="text" />
									...
									<input type="<c:out value="${inputType}" />" size="35" name="<c:out value='${controlConfigOption.option}' />" value="<c:out value='${controlConfigOption.value}' />" />
								</td>
							</c:when>
	...
	<tiles:insert definition=".page.footer">
		<c:if test="${monitorHelp != null}">
			<a name="setup"> </a>
			<c:out value="${monitorHelp}" escapeXml="false" />
		</c:if>
	</tiles:insert>
</html:form>