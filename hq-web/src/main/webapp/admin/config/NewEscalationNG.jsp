...
<jsu:script>
	...
	function addRow() {
	    ...
	    remDiv.innerHTML ='<a href="#" onclick="removeRow(this);"><img src="<s:url value='/images/tbb_delete.gif' />" height="16" width="46" border="0"  alt=""/></a>';
   ...
   function sendEditEscForm() {
        ...
        var url = '<s:url value="UpdateEscalation.action" />';
        if (hqDojo.byId('gad')) {
    ...
    function sendEscForm() {
		...
		var escFormSerial = $("EscalationForm").serialize();
        var url = '<s:url value="SaveEscalation.action" />';
        if (hqDojo.byId('gad')) {
           gadId = hqDojo.byId('gad').value;
        } else {
            gadId = '';
  ...
  <c:if test="${not empty alertDef}">
    <c:choose>
      <c:when test="${not empty Resource}">
        <s:hidden name="eid" id="eid" value="#attr.Resource.entityId" />
      </c:when>
      <c:otherwise>
        <s:hidden name="aetid" id="aetid" value="#attr.ResourceType.appdefTypeKey" />
      </c:otherwise>
    </c:choose>
  </c:if>
  <s:hidden name="escId" id="escId"/>
</s:form>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  <td class="ErrorBlock" width="9">
	<img src='<s:url value="/images/tt_error.gif" />' height="9" width="9" border="0" alt="" />
  </td>
...
<form action="<s:url value='SaveEscalation.action'/>" name="EscalationForm" id="EscalationForm" onchange="hideExample();" >
  ...
  <c:choose>
  <c:when test="${not empty Resource}">    
	<input type="hidden" id="eid" name="eid" value='<c:out value="${Resource.entityId}"/>' />
  </c:when>
  <c:otherwise>
	<input type="hidden" id="aetid" name="aetid" value='<c:out value="${ResourceType.appdefTypeKey}"/>' />
  </c:otherwise>
</c:choose> <c:choose>
  <c:when test="${not empty param.ad}">
    <input type="hidden" id="ad" name="ad" value='<c:out value="${param.ad}"/>' />
  ...
  <c:when test="${not empty param.gad}">
	...
	<input type="hidden" id="avi" name="avi" value='avi' />
        ...
        <tbody>
          <tr>
            <td style="padding-top:2px;padding-bottom:2px;">
				<span id="AlertEscalationOption"><input type="radio" id="allowPause" name="allowPause" value="true" />
				<fmt:message key="alert.config.escalation.allow.pause" />&nbsp;</span>
			</td>
          </tr>
          <tr>
            <td style="padding-top:2px;padding-bottom:2px;"><input
              type="radio" name="allowPause" id="allowPause" value="false" checked="true" /> <fmt:message
    ...
    <tr class="ListRow">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" border="0">
        <tbody>
          <tr>
            <td style="padding-top:2px;padding-bottom:2px;"><input
              type="radio" name="notifyAll" id="notifyAll" value="false" checked="true" /> <fmt:message
    ...
    <tr class="ToolbarContent"><!-- SET TOOLBAR -->
      <td>
        <tiles:insertTemplate template="/common/components/ActionButtonNG.jsp">
          <tiles:putAttribute name="labelKey" value="common.label.Next"/>
          <tiles:putAttribute name="buttonClick" value="return saveNewEscalation();"/>
...
<ul class="boxy">
  <c:forEach var="user" items="${AvailableUsers}" varStatus="status">
    <li class="ListRow"><input type="checkbox" name="users" id="users"
      value="<c:out value="${user.id}"/>"> <c:out value="${user.name}" /></input></li>
  </c:forEach>
</ul>
</div>
...
<c:if test="${not empty AvailableRoles}">
  ...
  <ul class="boxy">
    <c:forEach var="role" items="${AvailableRoles}" varStatus="status">
      <li class="ListRow"><input type="checkbox" name="roles" id="roles"
        ...
        <tbody>
          <tr>
            ...
            <div id="pauseTimeEdit" style="display:none;"><input
              type="radio" name="allowPauseThird" value="true" id="Third" /> <fmt:message
...
<br>
<br>
<input type="hidden" value="" id="creationTime" name="creationTime"/> 
<input type="hidden" value="" id="_version_" name="_version_"/> 
<input type="hidden" value="" id="notifyAll" name="notifyAll"/>
<input type="hidden" value="" id="modifiedTime" name="modifiedTime"/> 
<input type="hidden" value="" id="allowPause" name="allowPause"/> 
<input type="repeat" value="" id="repeat" name="repeat"/>
<input type="hidden" value="" id="escName" name="escName"/>
