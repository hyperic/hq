...
<jsu:script>
    ...
    function requestViewEscalation() {
        hqDojo.xhrGet({
            url: "<s:url value="JsonByEscalationId.action" />",
            content: {
        ...
        if (tmp.error) {
            ...
            for (i = 0; i < actions.length; i++) {
                ...
                <c:if test="${useroperations['modifyEscalation']}">
                    ...
                    td8.innerHTML = '<a href="#" onclick="removeRow(this);removeAction(' + actionId + ');"><img src="<s:url value='/images/tbb_delete.gif' />" height="16" width="46" border="0"  alt="" /></a>';
                </c:if>
            ...
            <c:if test="${useroperations['modifyEscalation']}">
                Sortable.create('viewEscalationUL', {
                    ...
                    onUpdate: function() {
                        var pars = "&id=" + id;
                        var url = '<s:url value="UpdateEscalationOrder.action"/>';
    ...
    function saveEscalation() {
        ...
        hqDojo.xhrPost({
            url: "<s:url value="UpdateEscalation.action"/>",
    ...
    function updateEscView(originalRequest) {
        ...
        hqDojo.byId('okCheck').innerHTML = '<img src="<s:url value='/images/tt_check.gif' />" height="9" width="9" border="0" alt="" />';
    ...
    function removeAction(id) {
        var url = '<s:url value="RemoveAction.action"/>';
        var pars = "id=" + id;
    ...
    function showErrorDisplay() {
        ...
        hqDojo.byId('okCheck').innerHTML = '<img src="/images/tt_error.gif" height="9" width="9" border="0" alt=""/>';
    ...
    function saveAddEscalation() {
        ...
        var url = '<s:url action="SaveAction.action"/>';
    ...
    <c:if test="${not empty alertDef}">
        <c:choose>
            <c:when test="${not empty Resource}">
                <s:hidden name="eid" value="#attr.Resource.entityId}"/>
            </c:when>
            <c:otherwise>
                <s:hidden name="aetid" value="#attr.ResourceType.appdefTypeKey}"/>
            </c:otherwise>
        </c:choose>
    </c:if>
    <s:hidden name="escId"/>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
            <td style="padding-right:5px;" id="okCheck">
				<img src='<s:url value="/images/tt_check.gif" />' height="9" width="9" border="0" alt="" />
            </td>
            <td width="98%">
                <div id="escMsg"></div>
            </td>
...
<form action='<s:url value="UpdateEscalation.action"/>'
      name="EscalationForm" id="EscalationForm" onchange="hideExample();"><input
        type="hidden" value="0" id="pid"> <input type="hidden" value="0" id="pversion"> 
...
<form name="viewEscalation" id="viewEscalation">
    ...
    <div>
        <div class="BlockTitle" style="position:relative;">
			...
			<img src='<s:url value="/images/icon_info.gif" />' style="position:absolute;right:3px;"
