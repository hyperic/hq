...
<jsu:script>
	...
	function removePortlet(name, label) {
		var portletUrl = "<s:url value="/app/dashboard/%{#attr.selectedDashboardId}/portlets/{portletName}"/>";
...
<s:a action="Resource" cssStyle="viewResUrl" >
...
<c:choose>
	...
	<table width="100%" border=0" cellspacing="0" cellpadding="0" >
		<tr>
			<td colspan="100%">
				<tiles:insertTemplate template="/portal/DashboardHeaderNG.jsp" />
			</td>
		</tr>
		<tr>
				...
				<c:choose>
					...
					<c:otherwise>
						<c:choose>
							...
							<c:otherwise>
								<c:set var="styleSpec" value="style='padding-left:5px;padding-right:16px;width:50%;'" />
							</c:otherwise>
						</c:choose>
					</c:otherwise>
				</c:choose>
									...
									<td valign="top" class="DashboardPadding">
										<c:choose>
											<c:when test="${narrow eq 'true'}">
												<tiles:insertDefinition name=".ng.dashContent.addContent.narrow" flush="true" />
											</c:when>
											<c:otherwise>
												<tiles:insertDefinition name=".ng.dashContent.addContent.wide" flush="true" />
											</c:otherwise>
										</c:choose>
									</td>
								</tr>
