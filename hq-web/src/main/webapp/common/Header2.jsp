 ...
 <jsu:script>
    function getUpdateStatus(opt) {
        if (opt == "<fmt:message key="header.Acknowledge"/>") {
            ...
            var updateUrl = '<s:url action="/Dashboard"/>';
            hqDojo.xhrPost({
         	 	url: updateUrl,
         	 	content: postData,
         	 	load: function(data) {
    ...
    function showRecentAlertResponse(response, args) {
      	...
      	if (refreshCount < 30) {
        	setTimeout( "refreshAlerts()", portlets_reload_time );
    	  	...
    	  	top.location.href = "<s:url action="/j_spring_security_logout"/>";
      	}
    }
...
<jsu:script onLoad="true">
    ...
    <div id="navTabContainer">
        ...
        <div id="resTab" class="tab">
        	<a href="/ResourceHub.do" onclick="useBreadcrumbHrefIfAvailable(this);">
        	...
        	<ul class="root">
                ...
                <li class="hasSubmenu">
                	<a href="">
                  	...
                  	<ul>
                    	<tiles:insertDefinition name=".toolbar.recentResources"/>
                  	</ul>
                </li>
        	</ul>
        </div>
        ...
        <div id="analyzeTab" class="tab">
        	...
        	<ul class="root">
              	<tiles:insertDefinition name=".header.optional.tabs">
                  	<tiles:putAttribute name="location" value="tracking"/>
              	</tiles:insertDefinition>
          	</ul>
        </div>
    ...
    <div class="ajaxLoading" style="display:none;" id="loading">
        <img src='<s:url value="/images/4.0/icons/ajax-loader.gif"/>' border="0" width="16" height="16"/>
    </div>
    ...
    <div id="headerLinks">
 	...
 	<a id="updateLink" href="javascript:updateDialog.show()">
 		<img src="<s:url value="/images/transmit2.gif" />" align="absMiddle" border="0" />                        
 	</a>
	</c:if>	
