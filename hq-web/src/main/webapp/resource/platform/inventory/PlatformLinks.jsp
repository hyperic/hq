...
<table border="0"><tr><td class="LinkBox">
    <c:if test="${canModify}">
    	<html:link action="/resource/platform/Inventory">
    		...
    		<fmt:message key="resource.platform.inventory.link.Configure"/><html:img page="/images/title_arrow.gif" width="11" height="9" alt="" border="0"/>
    	</html:link><br/>
    </c:if>
    <c:if test="${canRemove}" >
    ...
    <c:choose>
        <c:when test="${canCreateChild}" >
            <html:link action="/resource/server/Inventory">
            	...
            	<html:param name="eid" value="${resource.entityId}"/>
            	<fmt:message key="resource.platform.inventory.NewServerLink"/><html:img page="/images/title_arrow.gif" width="11" height="9" alt="" border="0"/>
            </html:link><br/>
            <html:link action="/resource/service/Inventory">
            	<html:param name="mode" value="new"/>
            	<html:param name="eid" value="${resource.entityId}"/>
            	<fmt:message key="resource.platform.inventory.NewServiceLink"/><html:img page="/images/title_arrow.gif" width="11" height="9" alt="" border="0"/>
            </html:link><br/>
        </c:when>
        <c:otherwise>
