...
<s:form action="/alerts/SetOpenNMSAction">
	<table width="100%" cellpadding="0" cellspacing="0" border="0" class="TableBottomLine">
  		...
  		<tr>
    		...
    		<c:if test="${OpenNMSForm.canModify}"  >
	    		<s:textfield name="server" value="%{#attr.server}"/>
	    	</c:if>
	    	<c:if test="${not OpenNMSForm.canModify}"  >
	    		<s:textfield name="server" value="%{#attr.server}" disabled="true" />
	    	</c:if>
    		</td>
    		<td class="BlockLabel"><fmt:message key="alert.config.edit.opennms.port"/></td>
    		<td class="BlockContent">
    			<c:if test="${OpenNMSForm.canModify}"  >
	    			<s:textfield name="port" value="%{#attr.port}"/>
	    		</c:if>
				<c:if test="${not OpenNMSForm.canModify}"  >
	    			<s:textfield name="port" value="%{#attr.port}" disabled="true"/>
	    		</c:if>
    		</td>
  		</tr>
	</table>
	<s:hidden theme="simple" value="%{#attr.ad}" name="ad"/>
	<s:hidden theme="simple" value="%{#attr.id}" name="id"/>
	...
	<c:if test="${OpenNMSForm.canModify}"  >
		<table width="100%" cellpadding="5" cellspacing="0" border="0" class="ToolbarContent" style="border-bottom: 1px solid #D5D8DE;">
  			<tr>
  				<td align="center">
  					<image src='<s:url value="/images/tbb_set.gif"/>' border="0" titleKey="FormButtons.ClickToOk" property="ok" style="padding-right: 5px;"/>
    				<image src='<s:url value="/images/tbb_remove.gif"/>' border="0" titleKey="FormButtons.ClickToDelete" property="delete" style="padding-left: 5px;"/>
    			</td>
    		</tr>
		</table>
	</c:if>
