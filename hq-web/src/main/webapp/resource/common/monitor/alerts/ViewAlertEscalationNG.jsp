...
<c:if test="${not empty escalation}">
	...
	<tiles:insertDefinition name=".header.tab">
  		<tiles:putAttribute name="tabKey" value="monitoring.events.MiniTabs.Escalation"/>
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="TableBottomLine" style="margin-bottom: 10px;">
  		...
  		<tr>
    		<td colspan="3"  class="BlockContent">
      			<tiles:insertTemplate template="/resource/common/monitor/alerts/config/ViewEscalationNG.jsp">
        			<tiles:putAttribute name="chooseScheme" value="false"/>
  		...
  		<c:if test="${alert.ackable && canTakeAction}">
  			<tr>
    			...
    			<td colspan="2" class="BlockContent" style="border-top: solid #D5D8DE 1px;">
        			<s:textarea name="ackNote" id="ackNote" value="%{#attr.ackNote}" cols="70" rows="4"/>	  
    			</td>
  			</tr>
  			<tr>
			 		...
			 		<c:if test="${escalation.pauseAllowed}">
	          		...
	          		<div style="text-align:left;">
				  		<tiles:insertTemplate template="/common/components/ActionButtonNG.jsp">
	  			     		<tiles:putAttribute name="labelKey" value="resource.common.alert.action.acknowledge.label"/>
	                 		<tiles:putAttribute name="buttonClick">hqDojo.byId('mode').setAttribute('value', '<fmt:message key="resource.common.alert.action.acknowledge.label"/>'); document.forms[0].action ='acknowledgeAlertAlertPortal.action';document.forms[0].submit();</tiles:putAttribute>
	              		</tiles:insertTemplate>
	          		</div>
	    		</td>
	  		</tr>
