...
<c:when test="${not empty chartDataKeys}">
  <fmt:message key="resource.common.monitor.visibility.request.wait"/><img src="<s:url value="/images/4.0/icons/ajax-loader.gif" />" alt="">
</c:when>
<c:otherwise>
...
<ul id="root" class="boxy">
	...
	<![endif]-->
	<script src="<s:url value='/static/js/dojo/1.5/dojo/dojo.js'/>" type="text/javascript"></script>
	<script src="<s:url value="/js/functions.js"/>" type="text/javascript"></script>
	<script src="<s:url value="/js/prototype.js"/>" type="text/javascript"></script>
	<script src="<s:url value="/js/effects.js"/>" type="text/javascript"></script>
	<script src="<s:url value="/js/lib/lib.js"/>" type="text/javascript"></script>
	<script>
		     ...
		     <c:when test="${xlib}">
		     		indicatorCharts[indicatorCharts.length] = {
		     			    ...
		     			    <c:if test="${not empty metric.childType}">
		     	     		...
		     	     		"timeToken" : "<c:out value="${timeToken}"/>"     	     		
		     		};
		     </c:when>
		     <c:otherwise>
					indicatorCharts[indicatorCharts.length] = {
		...
		var props = { 
				  ...
				  "eid" : "<c:out value="${eid}"/>",
				  "baseUrl" : "freshIndicatorCharts.action",
				  "displaySize" : <c:out value="${displaySize}"/>
		  };
