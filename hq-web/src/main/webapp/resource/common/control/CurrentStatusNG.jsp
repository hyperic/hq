...
<c:choose>
 ...
 <c:otherwise>
  <tiles:insertDefinition name=".header.tab">
   <tiles:putAttribute name="tabKey" value="${tabKey}"/>
   <tiles:putAttribute name="tabName" value="${controlCurrentStatus.action}"/>
  </tiles:insertDefinition>
 </c:otherwise>
</c:choose>
<!--  /  -->
...
<c:choose>
 <c:when test="${controlStatus eq 'In Progress'}">
  ...
  <c:if test="${section eq 'group'}">
   ...
   <c:set var="statusMsg"><c:out value="${statusMsg}"/>&nbsp;<s:a href="%{detailsLink}"><fmt:message key="resource.group.ControlStatus.Link.Details"/></s:a></c:set>
  </c:if>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
    ...
    <tr valign="top">
        ...
        <td width="30%" class="BlockContent"><span id="ctrlImg"><img src='<s:url value="%{#attr.statusImage}"/>' width="50" height="12" border="0"/></span>&nbsp;<span id="ctrlStatus"><c:out value="${statusMsg}" escapeXml="false"/></span></td>
...
<c:otherwise>
 <table width="100%" cellpadding="0" cellspacing="0" border="0">
  <tr>
   <td class="BlockContent" width="100%"><img src='<s:url value="/images/spacer.gif"/>' width="5" height="1" alt="" border="0"/><i><fmt:message key="resource.server.ControlStatus.Content.None"/></i></td>
  </tr>
  <tr>
