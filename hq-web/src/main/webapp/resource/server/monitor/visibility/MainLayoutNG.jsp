...
<c:choose>
    <c:when test="${canControl}">
        <tiles:insertDefinition name=".tabs.resource.server.monitor.visibility">
          <tiles:putAttribute name="resourceId" value="${Resource.id}"/>
          <tiles:putAttribute name="resourceType" value="${entityId.type}"/>
        </tiles:insertDefinition>
    </c:when>
    <c:otherwise>
        <tiles:insertDefinition name=".tabs.resource.server.monitor.visibility.nocontrol">
          <tiles:putAttribute name="resourceId" value="${Resource.id}"/>
          <tiles:putAttribute name="resourceType" value="${entityId.type}"/>
        </tiles:insertDefinition>
    </c:otherwise>
...
<table width="100%" class="MonitorBlockContainer">
  <tr>
    <td colspan="2" style="padding-bottom: 10px;">
      <s:form method="GET" action="metricsControlAction.action" onsubmit="makeNumeric()">
    ...
    <td valign="top">
      <c:choose>
        ...
        <c:when test="${isResourceMetrics}">
          <s:form id="filterMetricsForm" name="filterMetricsForm" action="resourceMetricsMonitorServerVisibility.action">
            <input type="hidden" name="eid" value="<c:out value="${eid}"/>">
...
<c:choose>
  <c:when test="${isCurrentHealth}">
    <tiles:insertTemplate template="/resource/common/monitor/visibility/IndicatorsNG.jsp">
      ...
      <tiles:putAttribute name="entityType" value="server"/>
    </tiles:insertTemplate>
  </c:when>
  <c:when test="${isResourceMetrics}">
    <tiles:insertTemplate template="/resource/server/monitor/visibility/ServerMetricsNG.jsp">
	  <tiles:putAttribute name="Resource" value="${Resource}"/>
