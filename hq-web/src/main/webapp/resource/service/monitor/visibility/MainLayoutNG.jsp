...
<c:choose>
 <c:when test="${ canControl }">
  <tiles:insertDefinition name=".tabs.resource.service.monitor.visibility">
   <tiles:putAttribute name="resourceId" value="${Resource.id}"/>
   <tiles:putAttribute name="resourceType" value="${entityId.type}"/>
  </tiles:insertDefinition>
 </c:when>
 <c:otherwise>
  <tiles:insertDefinition name=".tabs.resource.service.monitor.visibility.nocontrol">
   <tiles:putAttribute name="resourceId" value="${Resource.id}"/>
   <tiles:putAttribute name="resourceType" value="${entityId.type}"/>
  </tiles:insertDefinition>
 </c:otherwise>
  ...
  <tr>
    ...
    <c:choose>
      <c:when test="${isCurrentHealth}">
        <s:form id="ProblemMetricsDisplayForm" name="ProblemMetricsDisplayForm"
      ...
      <c:when test="${isResourceMetrics}">
        <s:form action="/resource/service/monitor/visibility/FilterServiceMetrics">
          <input type="hidden" name="eid" value="<c:out value="${eid}"/>">
          <tiles:insertTemplate template="/resource/service/monitor/visibility/CurrentHealthResourcesNG.jsp">
            <tiles:putAttribute name="mode" value="${mode}"/>
...
<c:choose>
  <c:when test="${isCurrentHealth}">
    <tiles:insertTemplate template="/resource/common/monitor/visibility/IndicatorsNG.jsp">
      ...
      <tiles:putAttribute name="entityType" value="service"/>
    </tiles:insertTemplate>
  </c:when>
  <c:when test="${isResourceMetrics}">
    <tiles:insertTemplate template="/resource/service/monitor/visibility/ServiceMetricsNG.jsp">
	  <tiles:putAttribute name="Resource" value="${Resource}"/>
      <tiles:putAttribute name="entityId" value="${entityId}"/>
      <tiles:putAttribute name="IsPlatformService" value="${IsPlatformService}"/>
