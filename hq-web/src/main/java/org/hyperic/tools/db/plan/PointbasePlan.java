...
class PointbasePlan extends Plan
{
    protected void printPlan(Connection conn, String cmd) throws SQLException {
        ...
        conn.createStatement().execute("DELETE FROM PLAN_QUERIES");
        conn.commit();
        
        conn.createStatement().execute("SET PLANONLY ON");
        conn.prepareStatement(cmd);
        conn.createStatement().execute("SET PLANONLY OFF");
        ...
        DBPlan.printResults(
            conn.createStatement().executeQuery("SELECT * FROM PLAN_QUERIES") );
    }
}
