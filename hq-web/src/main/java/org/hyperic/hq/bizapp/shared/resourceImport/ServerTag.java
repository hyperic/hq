               ...
               XmlTagEntryHandler, XmlTagExitHandler
{
    ...
    private XmlServerValue   servVal;  // Current working server
    private XmlPlatformValue platVal;  // Platform to add server to

    ServerTag(XmlPlatformValue platVal){
        super();
        String[] required, optional;
        
        required     = XmlServerValue.getRequiredAttributes();
        optional     = XmlServerValue.getOptionalAttributes();
    ...
    public XmlTagInfo[] getSubTags(){
        return (XmlTagInfo[])
            ArrayUtil.merge(super.getSubTags(), 
                new XmlTagInfo[] {
                    new XmlTagInfo(new ServiceTag(this.servVal), 
    ...
    public void enter(){
        if(this.servVal != null)
            throw new IllegalStateException("Invalid server state");

        this.servVal = new XmlServerValue();
        this.setEntityVal(this.servVal);
    }

    public void exit()
        throws XmlTagException
    {
        this.platVal.addServer(this.servVal);
        this.servVal = null;
        this.setEntityVal(this.servVal);
    }
}
