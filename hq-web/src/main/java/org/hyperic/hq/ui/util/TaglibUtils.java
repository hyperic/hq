...
public class TaglibUtils {
     ...
     * <code>null</code>, the variable will be set in the page scope.
     *
     * @param pageContext the page context
     * @param scope the scope of the variable ("application",
     * "session", "request", or "page")
     * @param var the name of the variable
     * @param value the value of the variable
     */
    public static void setScopedVariable(PageContext pageContext,
                                         String scope,
                                         String var,
                                         Object value) {
        if (scope == null) {
    ...
    public static String shortenPath(String path, int preChars, int postChars,
        ...
        StringBuffer ret = new StringBuffer(path.substring(0, preChars))
            .append("...")
            .append(path.substring(path.length() - postChars));
    ...
    public static String shortenPath(String path, int preChars, int postChars) {
        ...
        if (slash1 == -1) {
            if (slash2 == -1) return path;
            slash = '\\';
        }
        else {
            if (slash2 == -1) slash = '/';
            else if (slash1 < slash2) slash = '/';
            else slash = '\\';
        }
        ...
        try {
            ...
            String suffix = 
                path.substring(path.length() - postChars).substring(slash2);
            return prefix + "..." + suffix;
        } catch (StringIndexOutOfBoundsException e) {
            System.err.println("SBE on shortenPath(" + path + "," +
                               preChars + "," + postChars+")");
            throw e;
        }
    }
