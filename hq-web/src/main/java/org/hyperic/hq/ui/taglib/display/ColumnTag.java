 ...
 * objects. This describes a column of data in the SmartListTag. There can be
 * any (reasonable) number of columns that make up the list.
 * <p>
 * 
 ...
 * Usage:
 * <p>
 * <display:column property="title" title="College Title" width="33%"
 * href="/osiris/pubs/college/edit.page" paramId="OID" paramProperty="OID" />
 * 
 * Attributes:
 * <p>
 * 
 * property - the property method that is called to retrieve the information to
 * be displayed in this column. This method is called on the current object in
 * the iteration for the given row. The property format is in typical struts
 * format for properties (required)
 * 
 * title - the title displayed for this column. if this is omitted then the
 * property name is used for the title of the column (optional)
 * 
 * nulls - by default, null values don't appear in the list, by setting
 * viewNulls to 'true', then null values will appear as "null" in the list
 * (mostly useful for debugging) (optional)
 * 
 * width - the width of the column (gets passed along to the html td tag).
 * (optional)
 * 
 ...
 * attribute is not included, then no grouping is performed. (optional)
 * 
 * decorator - a class that should be used to "decorate" the underlying object
 * being displayed. If a decorator is specified for the entire table, then this
 * decorator will decorate that decorator. (optional)
 * 
 * autolink - if set to true, then any email addresses and URLs found in the
 * content of the column are automatically converted into a hypertext link.
 * 
 * href - if this attribute is provided, then the data that is shown for this
 * column is wrapped inside a <a href> tag with the url provided through this
 * attribute. Typically you would use this attribute along with one of the
 * struts-like param attributes below to create a dynamic link so that each row
 * creates a different URL based on the data that is being viewed. (optional)
 * 
 * paramId - The name of the request parameter that will be dynamically added to
 * the generated href URL. The corresponding value is defined by the
 * paramProperty and (optional) paramName attributes, optionally scoped by the
 * paramScope attribute. (optional)
 * 
 * paramName - The name of a JSP bean that is a String containing the value for
 * the request parameter named by paramId (if paramProperty is not specified),
 * or a JSP bean whose property getter is called to return a String (if
 * paramProperty is specified). The JSP bean is constrained to the bean scope
 * specified by the paramScope property, if it is specified. If paramName is
 * omitted, then it is assumed that the current object being iterated on is the
 * target bean. (optional)
 * 
 * paramProperty - The name of a property of the bean specified by the paramName
 * attribute (or the current object being iterated on if paramName is not
 * provided), whose return value must be a String containing the value of the
 ...
 * paramName attribute. If not specified, all scopes are searched. If paramName
 * is not provided, then the current object being iterated on is assumed to be
 * the target bean. (optional)
 * 
 * 
...
public class ColumnTag extends BodyTagSupport implements Cloneable {
	...
	private static Log log = LogFactory.getLog(ColumnTag.class.getName());
	...
	public int doEndTag() throws JspException {
		Object parent = TagSupport.findAncestorWithClass(this, TableTag.class);
