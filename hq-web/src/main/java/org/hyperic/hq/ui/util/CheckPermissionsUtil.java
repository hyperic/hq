...
public class CheckPermissionsUtil {
	private static Log log = LogFactory.getLog(CheckPermissionsUtil.class
			.getName());

	// Modifies the passed in list by removing resource that the user doesn't
	// have access to
	public static List<AppdefEntityID> filterEntityIdsByViewPermission(
			final int sessionId, final List<AppdefEntityID> appDefIds,
			final AppdefBoss adBoss) {
		...
		for (Iterator<AppdefEntityID> i = result.iterator(); i.hasNext();) {
			...
			try {
				AppdefResourcePermissions permissions = adBoss
						.getResourcePermissions(sessionId, adeId);
				...
				// Some problem occurred while checking this resource's
				// permissions
	...
	public static boolean canUserViewChart(int sessionId, String url,
		...
		try {
			...
			log.debug("Failed to decode url " + url,e);
		}
		...
		if (splitUrl.length == 2) {
			...
			try {
				...
				if (paramMap.containsKey(RESOURCE_ID_PARAM)
						&& paramMap.containsKey(RESOURCE_TYPE_PARAM)) {
					// If we've gotten this far we know there will be at least
					// one
					// value in both of these
					// lists. If we have to handle the case of multiple values,
					// this
					// would have to change
					// Hopefully, the design will be revisited at that point...
					String resourceType = paramMap.get(RESOURCE_TYPE_PARAM)
							.get(0);
					String resourceId = paramMap.get(RESOURCE_ID_PARAM).get(0);

					AppdefEntityID adeId = new AppdefEntityID(
							Integer.parseInt(resourceType),
							Integer.parseInt(resourceId));

					ids.add(adeId);
				} else if (paramMap.containsKey(EID_PARAM)) {
					AppdefEntityID adeId = new AppdefEntityID(paramMap.get(
							EID_PARAM).get(0));
				...
				if (ids.size() > 0) {
					// Now check it
					List<AppdefEntityID> list = CheckPermissionsUtil
							.filterEntityIdsByViewPermission(sessionId, ids,
									apBoss);
	...
	private static Map<String, List<String>> createParameterMap(
		...
		if (queryParameters.indexOf(QUERY_PARAM_DELIMITER) > -1) {
			String[] nameValuePairs = queryParameters
	...
	private static void processNameValuePair(Map<String, List<String>> map,
			String nameValuePair) {
		final String NAME_VALUE_PAIR_DELIMITER = "=";

		if (nameValuePair.indexOf(NAME_VALUE_PAIR_DELIMITER) > -1) {
			String[] nameAndValue = nameValuePair
					.split(NAME_VALUE_PAIR_DELIMITER);
			String name = nameAndValue[0];
			String value = nameAndValue.length == 2 ? nameAndValue[1] : ""; // Could
																			// be
																			// an
																			// empty
																			// value
