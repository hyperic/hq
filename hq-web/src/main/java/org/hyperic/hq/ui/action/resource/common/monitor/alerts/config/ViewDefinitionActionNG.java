 ...
 * 
 */
@Component("viewDefinitionActionNG")
public class ViewDefinitionActionNG extends BaseActionNG implements
		ViewPreparer {

	protected final Log log = LogFactory.getLog(ViewDefinitionActionNG.class
			.getName());
	@Autowired
	...
	public void doExecute(HttpServletRequest request) {
		...
		try {
			...
			request.setAttribute("canEditConditions", new Boolean(
					canEditConditions));
			List<AlertConditionBean> alertDefConditions = AlertDefUtil
					.getAlertConditionBeanList(sessionID, request,
							measurementBoss, acvList,
							EventConstants.TYPE_ALERT_DEF_ID.equals(adv
									.getParentId()));
			...
			try {
				...
				try {
					request.setAttribute(
							Constants.CAN_VIEW_RESOURCE_TYPE_ALERT_TEMPLATE_ATTR,
							false);
					// ...is this alert definition spawned from a resource alert
					// template?..
					if (adv.getParentId() != null && adv.getParentId() > 0) {
						// ...if so, check to see if we have permission to view
						// it...
						alertPermissionManager
								.canViewResourceTypeAlertDefinitionTemplate(subject);
						request.setAttribute(
								Constants.CAN_VIEW_RESOURCE_TYPE_ALERT_TEMPLATE_ATTR,
								true);
					}
