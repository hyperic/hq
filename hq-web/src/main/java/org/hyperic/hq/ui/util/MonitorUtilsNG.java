...
public class MonitorUtilsNG {
    ...
    public static void traceMetricDisplaySummaryMap(Log logger, Map metrics) {
        logger.trace("Dumping metric map (from " + MonitorUtilsNG.class.getName() + "):");
    ...
    public static void traceResourceTypeDisplaySummaryList(Log logger, List healths) {
        logger.trace("Dumping list of ResourceTypeDisplaySummary's (from " + MonitorUtilsNG.class.getName() + "):");
        if (healths == null) {
        ...
        try {
            for (Iterator iter = metrics.values().iterator(); iter.hasNext();) {
                ...
                for (Iterator m = metricList.iterator(); m.hasNext();) {
                    ...
                    String fmtString = LocalizedTextUtil.findDefaultText(Constants.UNIT_FORMAT_PREFIX_KEY + mds.getUnits(), userLocale);
