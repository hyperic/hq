...
public class MonitorUtilsNG {

	private static Log log = LogFactory.getLog(MonitorUtilsNG.class.getName());
	...
	public static void traceMetricDisplaySummaryMap(Log logger, Map metrics) {
		...
		for (Iterator categoryIter = metrics.keySet().iterator(); categoryIter
				.hasNext();) {
			...
			for (Iterator iter = metricList.iterator(); iter.hasNext();) {
				MetricDisplaySummary summaryBean = (MetricDisplaySummary) iter
						.next();
				++i;
	...
	public static void traceResourceTypeDisplaySummaryList(Log logger,
			List healths) {
		logger.trace("Dumping list of ResourceTypeDisplaySummary's (from "
				+ MonitorUtilsNG.class.getName() + "):");
		if (healths == null) {
		...
		for (Iterator iter = healths.iterator(); iter.hasNext();) {
			ResourceTypeDisplaySummary summaryBean = (ResourceTypeDisplaySummary) iter
					.next();
			++i;
			logger.trace("\t" + i + ":" + summaryBean);
		}
		...
		try {
			for (Iterator iter = metrics.values().iterator(); iter.hasNext();) {
				...
				for (Iterator m = metricList.iterator(); m.hasNext();) {
					...
					String fmtString = LocalizedTextUtil.findDefaultText(
							Constants.UNIT_FORMAT_PREFIX_KEY + mds.getUnits(),
							userLocale);

					if (fmtString != null) {
						// this means that there's a whole song and dance for
						// formatting this type of thing
						// a certain way (hopefully in a way that some decendent
						// of java.test.Format will
						// help with)

						if (mds.getUnits().equals(
								MeasurementConstants.UNITS_EPOCH_MILLIS)) {
							DateSpecifics specs = new DateSpecifics();

							specs.setDateFormat(new SimpleDateFormat(fmtString,
									userLocale));
