...
public class AvailabilityDataDAO
    ...
    AvailabilityDataRLE findAvail(DataPoint state) {
        ...
        List list = getSession().createQuery(sql).setLong("startime", state.getTimestamp())
            .setInteger("meas", state.getMetricId().intValue()).list();
        if (list.isEmpty()) {
            return null;
        }
    ...
    AvailabilityDataRLE findAvailAfter(DataPoint state) {
        String sql = new StringBuilder().append("FROM AvailabilityDataRLE").append(
            ...
            .toString();
        List list = getSession().createQuery(sql).setLong("startime", state.getTimestamp())
            .setInteger("meas", state.getMetricId().intValue()).setMaxResults(1).list();
        if (list.isEmpty()) {
            return null;
        }
        return (AvailabilityDataRLE) list.get(0);
