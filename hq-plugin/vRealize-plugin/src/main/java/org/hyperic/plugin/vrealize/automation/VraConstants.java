/**
 *
 */
package org.hyperic.plugin.vrealize.automation;

/**
 * @author imakhlin
 *
 */
public final class VraConstants {

    public static final String TYPE_LOAD_BALANCER = "Load Balancer";
    public static final String TYPE_VRA_VA_LOAD_BALANCER = "vRealize Automation VA Load Balancer";
    public static final String TYPE_VRA_LOAD_BALANCER = "vRealize Automation Load Balancer";
    public static final String TYPE_VRA_APPLICATION = "vRealize Automation Application";
    public static final String TYPE_SSO = "SSO";
    public static final String TYPE_VRA_VSPHERE_SSO = "vSphere SSO";
    public static final String TYPE_VCO = "vCenter Orchestrator"; // former VCO
    public static final String TYPE_VRA_VCO = "vCenter Orchestrator App Server";
    public static final String TYPE_VRA_SERVER_GROUP = "vRealize Automation Server Group";
    public static final String TYPE_VRA_SERVER = "vRealize Automation Server";

    public static final String TYPE_VRA_IAAS_WEB = "vRealize Automation IaaS Web";
    public static final String TYPE_VRA_IAAS_WEB_GROUP = "vRealize Automation IaaS Web Group";
    public static final String TYPE_VRA_IAAS_WEB_LOAD_BALANCER = "vRealize Automation IaaS Web Load Balancer Service";
    public static final String TYPE_VRA_IAAS_MANAGER_LOAD_BALANCER = "vRealize Automation Manager Load Balancer";
    public static final String TYPE_VRA_IAAS_MANAGER_SERVER = "vRealize Automation Manager Server";

    public static final String KEY_APPLICATION_NAME = "application.name";

    public static final String PROP_EXTENDED_REL_MODEL = "extended-relationship-model";
    public static final boolean CREATE_IF_NOT_EXIST = true;
}
