...
public class ClusterDetect {
    ...
    private static final Pattern CLUSTER_NAME_PATTERN = Pattern.compile("Listing properties for '(.*)'", Pattern.CASE_INSENSITIVE);
    ...
    public static Properties getMssqlClusterProps(String instanceName) {
        ...
        String sqlServerResource = getSqlServerResource(clusterResources, instanceName);
        ...
        String networkName = getNetworkNameFromResource(sqlServerResource, clusterResources);
    ...
    private static String getSqlServerResource(String clusterResources, String instanceName) {
        Pattern sqlServerResourcePattern = Pattern.compile("^(\\w+)\\s+(.+\\S)\\s+InstanceName\\s+" + instanceName, Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
    ...
    private static String getNetworkNameFromResource(String networkNameResource, String clusterResources) {
        Pattern sqlNetworkNamePattern = Pattern.compile(networkNameResource + "\\s+VirtualServerName\\s+(\\S+)", Pattern.CASE_INSENSITIVE);
