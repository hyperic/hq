...
import static org.hyperic.plugin.openstack.OpenstackConstants.KEYSTONE_ALL_PTQL;
import static org.hyperic.plugin.openstack.OpenstackConstants.KEYSTONE_ALL_SERVICE;
import static org.hyperic.plugin.openstack.OpenstackConstants.KEYSTONE_INSTALL_PATH;
...
import static org.hyperic.plugin.openstack.OpenstackUtil.getKeystoneAllPtql;
import static org.hyperic.plugin.openstack.OpenstackUtil.getKeystonePtql;
...
/**
 * Class identifies Openstack Keystone Processes to be monitored.
 */
public class OpenstackKeystoneServerDetector extends ServerDetector implements
		AutoServerDetector {

	private static Log logger = LogFactory
			.getLog(OpenstackKeystoneServerDetector.class);

	private boolean isKeystoneAllServiceAvailable = false;
	...
	public List getServerResources(ConfigResponse platformConfig)
		...
		String keystoneService = getAvailableKeystoneService();

		logger.debug("Keystone Server monitoring PTQL:" + keystoneService);

		// Server Resources created if one of monitored process is available.
		if (keystoneService != null) {
			ServerResource server = createServerResource(KEYSTONE_INSTALL_PATH);
			// server.setName(server.getName());
			String platformName = getPlatformName(server.getName());
			server.setName(platformName + KEYSTONE_SERVER);
			ConfigResponse productConfig = new ConfigResponse();
			productConfig.setValue(PROCESS_QUERY, keystoneService);
	 ...
	 * at least one of monitored Keystone process running.
	 * 
	 * @return String
	 */
	private String getAvailableKeystoneService() {
		List<String[]> keystoneServicePtqls = getKeystonePtql();
		for (String[] keystoneServicePtql : keystoneServicePtqls) {
			for (int i = 0; i < keystoneServicePtql.length; i++) {
				long[] pids = getPids(keystoneServicePtql[i]);
				if (pids != null && pids.length > 0) {
					return keystoneServicePtql[i];
				}
			}
		}
		return null;
	...
	protected List discoverServices(ConfigResponse serverConfig)
			throws PluginException {
		logger.debug("[OpenstackKeystoneServerDetector discoverServices] serverConfig="
				+ serverConfig);

		List services = new ArrayList();

		String keystoneAllServicePtql = getServicePtql(getKeystoneAllPtql(),
				KEYSTONE_ALL_PTQL);
		if (isServiceAvailable(KEYSTONE_ALL_PTQL, isKeystoneAllServiceAvailable)) {
			services.add(getService(KEYSTONE_ALL_SERVICE,
					keystoneAllServicePtql));
			logProcess(KEYSTONE_ALL_SERVICE, keystoneAllServicePtql);
		}
