...
public class EventHandlerImpl implements EventHandler {
    ...
    private static final Log logger = LogFactory.getLog(EventHandlerImpl.class.getName());
    ...
    /**
     * Events In the latestPage and
     * Events Before Latest Page/PreviousEvents.
    ...
    public void handleEvents(EventHistoryCollector ehc, int pageSize) {
        if (ehc != null) {
            try {
                logger.debug("Events In the latestPage are : ");
                handleEvents(ehc.getLatestPage());

                logger.debug("\nBefore Latest Page/PreviousEvents:");
                ehc.resetCollector();
    ...
    /** I will prob rip all of the following code out, It's steve's but I don't think we'll go this route */

    public void handleUpdate(UpdateSet update) {
        ArrayList vmUpdates = new ArrayList();
        PropertyFilterUpdate[] filterUpdates = update.getFilterSet();
        for (int i = 0; i < filterUpdates.length; i++) {
