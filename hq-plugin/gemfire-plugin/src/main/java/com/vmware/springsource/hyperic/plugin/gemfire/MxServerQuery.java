...
log.debug("find services first");
ServerDetector sd = getServerDetector();
log.debug("ServerDetector is of type "+sd.getClass().getName());
        Map servicePlugins = sd.getServiceInventoryPlugins();

        if (servicePlugins == null) {
        log.debug("servicePlugins == null");
            return;
        }
...
log.debug("findServices second");
        ...
        try {
            ...
            log.debug("mServer.queryNames OKay");
...
log.debug("Looking for object "+name);
            if ((matcher != null) &&
