...
log.debug("configureMxURL");
    ...
    protected boolean configureLocalMxURL(ConfigResponse config,
        ...
        log.debug("XXXXXXXXXXXXXXXXXXXXXXXX  configureLocalMxURL");
    ...
    //For some stupid reason, this is being called twice
    protected String getProcQuery(String path) {
        log.debug("getProcQuery start");
        ...
        */
        log.debug("Returning home property");
        return homeProp;
    }
...
log.debug("matches source="+source+" regex="+regex);
        return Pattern.compile(regex).matcher(source).find();
    }

    //This is probably what is being called
    protected List getServerProcessList()
    {
        List procs = new ArrayList();
        long[] pids = getPids(getProcQuery());
        log.debug("getSeverProcessList: "+" matched " + pids.length + " processes");
...
log.debug("isInstallTypeVersion dir="+dir);
    ...
    protected  ServerResource getServerResource(MxProcess process) {
        ...
        log.debug("getServerResource  PROC_QUERY: "+query);
...
log.debug("getServerResources");
    ...
    protected List discoverMxServices(MBeanServerConnection mServer,
        ...
        if (objName != null) {
            ...
            log.debug("MalformedObjectNameException");
...
log.debug("finding services");
    ...
    protected List discoverServices(ConfigResponse serverConfig)
        ...
        log.debug("discoverServices "+serverConfig.toString());
        JMXConnector connector;
        MBeanServerConnection mServer;
    
        try {
            connector = MxUtil.getMBeanConnector(serverConfig.toProperties());
            log.debug("serverConfig.properties "+connector.toString());
            mServer = connector.getMBeanServerConnection();
        } catch (Exception e) {
        log.debug("Got exception while getting connector/mServer");
	...
	throws PluginException {
        log.debug("XXXXXXXXXXXXXXXXXXXXXXXX  discoverServiceTypes");
    	 JMXConnector connector;
         MBeanServerConnection mServer;
