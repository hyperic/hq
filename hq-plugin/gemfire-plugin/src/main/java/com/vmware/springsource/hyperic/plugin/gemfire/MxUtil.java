...
public class MxUtil {
    ...
    //Super hack.  Since the ObjectName we need to use
    private static ObjectName gemFireStatObjName = null;
    ...
    /*
        Commenting out this method as it depends on a library that I can't find!!!
        We'll see if any of the debug messages come out, and if so then track it down.
     */
    static Double getJSR77Statistic(MBeanServerConnection mServer,
        ...
        throws MalformedURLException,
               ...
               PluginException {
        long value=0;
        log.debug("In getJSR77Statistic.  Returning 0");
...
/*
               ...
               PluginException
    {
        ...
        int ix = name.indexOf('.');
        if (ix == -1) { throw new MetricInvalidException("Missing composite key"); }
    ...
    /*
        Primary getValue mathod.  Parses "metric" and calls the more fundamental method.
     */
    ...
    /*
        This is the real get connection called by the connect method.
        
        Once we get the connection, let's try to call the connectToServer method, and get the MBean fully
        initialized for GemFire.
     */
    public static JMXConnector getMBeanConnector(Properties config)
        throws MalformedURLException,
               IOException
    {
log.debug("getMBeanConnector(Properties config)");
        String jmxUrl = config.getProperty(MxUtil.PROP_JMX_URL);
        Map map = new HashMap();
    ...
    log.debug("disconnect");
        ...
        if (obj != null)
        {
            ...
            {
                log.debug("Previous connector (" + address(connector) + ") already closed (" + e.getMessage() + ") for: " + jmxUrl);
            }
        }        
    }

    /*
        This seems like the initial entry point if a connection doesn't exit.
        It will close any active connection and then connect to the server and shove it into the cache.
     */
    ...
    /*
        This is the method called to get the ServerConnection.
     */
    public static MBeanServerConnection getMBeanServer(Properties config)
        throws MalformedURLException,
               IOException
    {
log.debug("getMBeanServer.  Properties: "+config.toString());
        String jmxUrl = config.getProperty(MxUtil.PROP_JMX_URL);
        ...
        if (instance != null)
        {
            ...
            if (usernameChanged || passwordChanged)
            {
                ...
                {
                    String diff = "";
                    if (usernameChanged) { diff += "user:" + diff(username, jmxUsername); }
                    if (passwordChanged)
                    {
                        if (diff.length() != 0) { diff += ","; }
                        diff += "pass:" + diff(mask(password), mask(jmxPassword));
                    }
                    log.debug("Credentials changed (" + diff + ") reconnecting cached connection for: " + jmxUrl);                    
                }
               ...
               PluginException
    {
        ...
        log.debug("getValue(Properties config, String objectName, String attribute)");
        ObjectName objName = new ObjectName(objectName);
        MBeanServerConnection mServer = getMBeanServer(config);

        if (attribute.startsWith(STATS_PREFIX))
        {
        log.debug("WRONG! get the JSR77 stat");
            return getJSR77Statistic(mServer, objName, attribute);
        }
        else if (attribute.startsWith(COMPOSITE_PREFIX))
        {
        log.debug("WRONG! get a composit metric");
            return getCompositeMetric(mServer, objName, attribute);
        }
        else
        {
        log.debug("Get the GemFire stats by running going through all the steps each time.");

            log.debug("Invoking connectToSystem");
            objName = (ObjectName)mServer.invoke(objName, "connectToSystem", null, null);
            log.debug("connectToServer returned: "+objName);
            
            log.debug("Invoke manageCacheServers on distributed system bean");
            ObjectName manageCacheServersObjNames[] = (ObjectName[])mServer.invoke(objName, "manageCacheServers", null, null);
            if(manageCacheServersObjNames!=null)
            {
                //We'll assume 1 returned server.  May need to modify for more servers
                if(manageCacheServersObjNames.length >= 1)
                {
                    log.debug("manageCacheServers returned "+manageCacheServersObjNames[0]+" items.  (We're using only the first)");
                    
                    log.debug("calling manageStats");
                    ObjectName statObjNames[] = (ObjectName[])mServer.invoke(manageCacheServersObjNames[0], "manageStats", null, null);
                    if(statObjNames != null)
                    {
                        //Handle "composit" attribute name that is really category,attributeName
                        int divIndex = attribute.indexOf(",");
                        String realAttribute = attribute;
                        String categoryName = "cachePerfStats";
                        if(divIndex > 0) //separate out the category from the attribute name
                        {
                            log.debug("found complex pattern for attribute name ... separating");
                            categoryName = attribute.substring(0,divIndex);
                            realAttribute = attribute.substring(divIndex+1);
                            log.debug("category="+categoryName+" attribute="+realAttribute);
                        }
                        for(ObjectName statName : statObjNames)
                        {
                            if(statName.toString().indexOf(categoryName)>0)
                            {
                                log.debug("Found "+categoryName+" ... calling refresh");
                                mServer.invoke(statName, "refresh", null, null);
                                //Get the attribute
                                log.debug("Getting the attribute: "+realAttribute);
                                retVal =  mServer.getAttribute(statName, realAttribute);
                                log.debug("return value is: "+retVal);
                                break;
                            }
                        }
                    }
                    else { log.debug("Did not find the stat object names"); }
                }
                else { log.debug("manageCacheServers didn't return null, but the length is 0"); }
            }
            else  { log.debug("manageCacheServers returned null"); }
        }
        if(retVal == null)
        {
            log.error("Did not find a return value");
               ...
               PluginException
    {
        ...
        catch (RemoteException e)              { throw unreachable(config, e);           }
        catch (MalformedObjectNameException e) { throw invalidObjectName(objectName, e); }
        catch (InstanceNotFoundException e) {
            throw objectNotFound(objectName, e);
        } catch (ReflectionException e) {
