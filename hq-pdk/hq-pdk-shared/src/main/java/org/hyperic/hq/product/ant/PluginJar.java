...
public class PluginJar
    ...
    public String getDir() {
        if (this.pluginDir == null) {
            ...
            String dir = home + "/hq-plugins/" + getName();
    ...
    public static void validatePluginXML(String dir, String name) throws BuildException {
    ...
    public void execute() throws BuildException {
        ...
        boolean validate = getProperty("pluginxml.validate", "true").equals("true");// XXX

        String classesMatch = "*/" + StringUtil.replace(packageName.substring(4), ".", "/");
        ...
        if (new File(getDir(), "src/main/resources/etc").exists()) {
            FileSet set = new FileSet();
            set.setDir(new File(getDir(), "src/main/resources/"));
            set.setIncludes("etc/**");
            addFileset(set);
        }
        
        if (new File(getDir(), "src/main/resources/mibs").exists()) {
            FileSet set = new FileSet();
            set.setDir(new File(getDir(), "src/main/resources/"));
            set.setIncludes("mibs/**");
            addFileset(set);
        }
        
        String[] dirs = { "etc", "mibs", "lib", "scripts", "hqu" };
